...
`).map(i=>i.replace(/^[-*•]\s*/,"").trim()).filter(i=>i.length>0);if(l.length===0)return g("Rincian perubahan tidak boleh kosong!");j("Menyimpan catatan pembaruan...");const n={id:Fe||"log-"+Date.now().toString(36),version:e.startsWith("v")?e:"v"+e,category:t,date:a,title:r,items:l,updatedAt:new Date().toISOString()};if(o.changelog=o.changelog||[],Fe){const i=o.changelog.findIndex(d=>d.id===Fe);i!==-1?o.changelog[i]=n:o.changelog.unshift(n)}else o.changelog.unshift(n);o.deletedChangelogIds&&Array.isArray(o.deletedChangelogIds)&&(o.deletedChangelogIds=o.deletedChangelogIds.filter(i=>i!==n.id&&i!==n.version));try{await q(["changelog","deletedChangelogIds"]),g("Catatan pembaruan berhasil dipublikasikan secara real-time!","success"),He=!1,Fe=null,_e()}catch(i){g("Gagal menyimpan log pembaruan: "+i.message,"error")}finally{C()}},to=e=>{const a=st(o).find(s=>s.id===e||s.version===e);if(!a)return;const r=a.version||a.title||"ini";ve(`Hapus catatan pembaruan versi "${r}" dari daftar log toko?`,async()=>{j("Menghapus catatan...");try{o.changelog=(o.changelog||[]).filter(l=>l.id!==e&&l.version!==e),o.deletedChangelogIds=Array.isArray(o.deletedChangelogIds)?o.deletedChangelogIds:[];const s=a.id||e;o.deletedChangelogIds.includes(s)||o.deletedChangelogIds.push(s),a.version&&!o.deletedChangelogIds.includes(a.version)&&o.deletedChangelogIds.push(a.version),await q(["changelog","deletedChangelogIds"]),g(`Catatan pembaruan ${r} berhasil dihapus!`,"success"),_e()}catch(s){g("Gagal menghapus catatan: "+s.message,"error")}finally{C()}},"Ya, Hapus","Konfirmasi Hapus Log")},ao=()=>{const e=st(o);if(e.length<=5)return g(`Daftar log masih ringkas (${e.length} versi), belum perlu pembersihan.`,"info");const t=e.slice(5),a=t.length;ve(`Pangkas ${a} catatan log pembaruan terlama dan hanya sisakan 5 versi terbaru? Tindakan ini merapikan daftar log toko agar tidak menumpuk spam.`,async()=>{j("Memangkas catatan lama...");try{const r=new Set;t.forEach(s=>{s.id&&r.add(s.id),s.version&&r.add(s.version)}),o.changelog=(o.changelog||[]).filter(s=>!r.has(s.id)&&!r.has(s.version)),o.deletedChangelogIds=Array.isArray(o.deletedChangelogIds)?o.deletedChangelogIds:[],r.forEach(s=>{o.deletedChangelogIds.includes(s)||o.deletedChangelogIds.push(s)}),await q(["changelog","deletedChangelogIds"]),g(`Berhasil membersihkan ${a} log lama! Tersisa 5 versi terbaru.`,"success"),_e()}catch(r){g("Gagal memangkas log: "+r.message,"error")}finally{C()}},"Ya, Pangkas Log","Pangkas Log Terlama")},so=()=>{if((o.deletedChangelogIds||[]).length===0)return g("Tidak ada log bawaan yang terhapus.","info");ve("Pulihkan kembali seluruh catatan log rilis sistem yang pernah dihapus?",async()=>{j("Memulihkan catatan log...");try{o.deletedChangelogIds=[],await q(["deletedChangelogIds"]),g("Seluruh log pembaruan bawaan berhasil dipulihkan!","success"),_e()}catch(t){g("Gagal memulihkan catatan: "+t.message,"error")}finally{C()}},"Ya, Pulihkan","Pulihkan Log Bawaan")};window.rAdmChangelog=_e;window.toggleChangelogForm=Xr;window.editChangelogEntry=Zr;window.saveChangelogEntry=eo;window.deleteChangelogEntry=to;window.pruneOldChangelogs=ao;window.restoreDefaultChangelogs=so;export{as as A,Ke as a,Ve as b,Dr as c,S as d,Er as e,_r as f,Nr as g,ss as h,no as i,Z as j,ba as k,lo as l,fa as m,ut as n,mt as o,ca as p,Xe as q,po as r,Zs as s,co as t,Oe as u,$a as v};

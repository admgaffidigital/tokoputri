import{d as H,_ as R}from"./module-member-CGh5W0Mi.js";import{a as h,e as c,t as g,i,b as S,r as K,f as V,x as de,u as ce}from"./module-print-DoqR8CYY.js";import{f as pe}from"./vendor-firebase-core-D2OF5R23.js";import"./vendor-firebase-db-BIUZcnOd.js";let p=[],j="",P="",x={name:"",phone:"",isMember:!1,memberId:null,isNewTempo:!1},u="cash",T=0,M=0,$="",F=null;const w=e=>Math.max(0,parseInt(e)||0),n=e=>V(e),q=()=>p.reduce((e,t)=>e+t.subtotal,0),f=()=>Math.max(0,q()-w(M)),G=()=>Math.max(0,T-f()),O=e=>(e.subtotal=Math.max(0,e.price*e.qty-w(e.discount)),e),be=()=>{const e=new Date,t=a=>String(a).padStart(2,"0");return`POS-${e.getFullYear()}${t(e.getMonth()+1)}${t(e.getDate())}-${Date.now().toString(36).toUpperCase()}`},me=()=>{window.__posBarcodeFn&&document.removeEventListener("keydown",window.__posBarcodeFn),window.__posBarcodeFn=e=>{const t=document.activeElement?.tagName?.toLowerCase();if(!(t==="input"||t==="textarea"||t==="select"))if(e.key==="Enter"){if($.length>=3){const a=$.trim().toLowerCase(),o=(h.products||[]).find(s=>s&&s.isActive!=="false"&&s.isActive!==!1&&(s.barcode&&s.barcode.toLowerCase()===a||s.sku&&s.sku.toLowerCase()===a||s.id&&String(s.id).toLowerCase()===a));if(o)B(o.id),g(`Ditambahkan: ${o.name}`,"success");else{const s=c("pos-search-input-d");s&&(s.value=$,j=$,C()),g("Barcode tidak ditemukan di katalog","warning")}$=""}}else e.key.length===1&&($+=e.key,clearTimeout(F),F=setTimeout(()=>{$=""},150))},document.addEventListener("keydown",window.__posBarcodeFn)},B=e=>{const t=(h.products||[]).find(o=>o&&String(o.id)===String(e));if(!t)return;const a=p.find(o=>String(o.id)===String(e));if(a)a.qty+=1,O(a);else{const o=parseFloat(t.price)||0;p.push(O({id:t.id,name:t.name,price:o,qty:1,discount:0,subtotal:o}))}k()},Y=(e,t)=>{const a=p.find(o=>String(o.id)===String(e));a&&(a.qty=Math.max(1,a.qty+t),O(a),k())},J=(e,t)=>{const a=p.find(o=>String(o.id)===String(e));a&&(a.qty=Math.max(1,w(t)),O(a),k())},W=(e,t)=>{const a=p.find(o=>String(o.id)===String(e));a&&(a.discount=Math.min(w(t),a.price*a.qty),O(a),k())},X=e=>{p=p.filter(t=>String(t.id)!==String(e)),k()},Z=()=>{p=[],M=0,k()},C=()=>{const e=(h.products||[]).filter(l=>{if(!l||l.isActive==="false"||l.isActive===!1||P&&l.category!==P)return!1;if(j){const b=j.toLowerCase();return(l.name||"").toLowerCase().includes(b)||(l.barcode||"").toLowerCase().includes(b)||(l.sku||"").toLowerCase().includes(b)}return!0}),a=["Semua",...new Set((h.products||[]).filter(l=>l&&l.isActive!=="false"&&l.category).map(l=>l.category))].map(l=>{const b=l==="Semua",r=b?!P:P===l;return`<button onclick="window.posCatFilter('${i(b?"":l)}')" class="shrink-0 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border transition-all ${r?"text-white border-transparent":"bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700"}" style="${r?"background:var(--color-primary)":""}">${i(l)}</button>`}).join(""),o=e.length===0?'<div class="col-span-full flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-600"><i class="fa-solid fa-box-open text-4xl mb-3"></i><p class="font-semibold text-sm">Produk tidak ditemukan</p></div>':e.map(l=>{const b=l.img?de(l.img,"w200-rw"):"",r=p.find(y=>String(y.id)===String(l.id));return`<button onclick="window.posAddToCart('${i(String(l.id))}')" class="relative flex flex-col bg-white dark:bg-slate-800 border rounded-2xl p-2.5 text-left transition-all duration-150 hover:-translate-y-0.5 hover:shadow-md active:scale-95 overflow-hidden ${r?"border-[var(--color-primary)] shadow-sm":"border-slate-200 dark:border-slate-700"}">
                ...
                <div class="flex flex-col items-center gap-1 shrink-0">
                    ...
                    <p class="text-[10px] font-black" style="color:var(--color-primary)">${n(o.subtotal)}</p>
            ...
            </div>`}).join("");S("pos-cart-items",e),K("pos-subtotal",n(q()));const t=c("pos-global-disc");t&&document.activeElement!==t&&(t.value=M||""),K("pos-total-amount",n(f()));const a=c("pos-pay-btn");a&&(a.disabled=p.length===0,a.innerHTML=p.length>0?`<i class="fa-solid fa-cash-register mr-2"></i>BAYAR — ${n(f())}`:'<i class="fa-solid fa-cash-register mr-2"></i>BAYAR')},ee=()=>{if(p.length===0){g("Keranjang masih kosong!","warning");return}x={name:"",phone:"",isMember:!1,memberId:null,isNewTempo:!1},u="cash",T=0,document.body.insertAdjacentHTML("beforeend",`
      ...
      <div class="bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md max-h-[92vh] flex flex-col">
        <div class="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0">
          <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Total: <span class="font-black" style="color:var(--color-primary)">${n(f())}</span></p>
          </div>
    ...
    </div>`),ae("cash")},A=()=>c("pos-pay-modal")?.remove(),te=(e,t,a)=>{a.forEach(o=>{const s=c(`${e}-${o}`);s&&(o===t?(s.style.background="var(--color-primary)",s.style.color="white",s.style.borderColor="var(--color-primary)"):(s.style.removeProperty("background"),s.style.removeProperty("color"),s.style.removeProperty("border-color")))})},ae=e=>{const t=c("pos-pay-detail");if(!t)return;const a=f(),o=`<div class="flex justify-between py-2 border-b border-slate-100 dark:border-slate-800 mb-3 text-sm"><span class="text-slate-500">Total Tagihan</span><span class="font-black" style="color:var(--color-primary)">${n(a)}</span></div>`;if(e==="cash")t.innerHTML=`${o}<label class="text-[10px] text-slate-500 font-semibold">Nominal Bayar (Rp)</label>
        ...
        <div class="flex justify-between mt-2 p-3 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl"><span class="text-sm font-bold text-emerald-700 dark:text-emerald-400">Kembalian</span><span id="pos-change-display" class="font-black text-sm text-emerald-700 dark:text-emerald-400">${n(0)}</span></div>`;else if(e==="qris"){const s=h.payment?.qrisUrl||"";t.innerHTML=`${o}${s?`<div class="flex justify-center"><img src="${i(s)}" class="w-44 h-44 object-contain rounded-xl border" alt="QRIS"></div><p class="text-center text-xs text-slate-500 mt-2">Scan QRIS untuk pembayaran</p>`:'<p class="text-center text-xs text-rose-500 font-semibold p-3 bg-rose-50 rounded-xl">QRIS belum diatur — buka Pengaturan → QRIS Pay</p>'}`}else if(e==="transfer"){const s=(h.banks||[]).filter(d=>d&&d.name);t.innerHTML=`${o}<label class="text-[10px] text-slate-500 font-semibold block mb-1">Pilih Rekening Tujuan</label>
        ...
        <input id="pos-cust-phone" type="tel" placeholder="No. HP *" required class="w-full border border-amber-300 dark:border-amber-600 rounded-xl px-3 py-2.5 text-sm bg-white dark:bg-slate-800 focus:outline-none">`))},N=e=>{u=e,te("pos-pay",e,["cash","qris","transfer","tempo"]),ae(e)},oe=e=>{T=w(e);const t=c("pos-change-display");t&&(t.textContent=n(G()))},re=()=>{const e=c("pos-cust-phone")?.value?.trim();if(!e){g("Masukkan nomor HP","warning");return}const t=e.replace(/\D/g,""),a=(h.customers||[]).find(s=>s&&s.phone&&s.phone.replace(/\D/g,"").endsWith(t)),o=c("pos-member-result");o&&(a?(x.name=a.name||"",x.memberId=a.id||a.phone,o.innerHTML=`<div class="flex items-center gap-2 p-2.5 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl border border-emerald-200"><i class="fa-solid fa-circle-check text-emerald-500"></i><div><p class="text-xs font-bold text-emerald-700">${i(a.name)}</p><p class="text-[10px] text-emerald-600">Member Terverifikasi ✓</p></div></div>`):o.innerHTML='<p class="text-xs text-rose-500 font-semibold p-2 bg-rose-50 rounded-xl border border-rose-200"><i class="fa-solid fa-circle-xmark mr-1"></i>Tidak ditemukan di database member</p>')},le=async()=>{if(p.length===0){g("Keranjang kosong!","warning");return}const e=c("pos-cust-name")?.value?.trim()||"Pelanggan Umum",t=c("pos-cust-phone")?.value?.trim()||"";if(x.isNewTempo&&!t){g("No. HP wajib diisi untuk tempo!","warning");return}if(u==="cash"&&(T=w(c("pos-paid-input")?.value||0),T<f())){g(`Uang kurang! Minimal ${n(f())}`,"warning");return}x.name=e,x.phone=t;const a=u==="tempo"?w(c("pos-dp-input")?.value||0):0,o=u==="transfer"&&c("pos-bank-sel")?.value||"",s=c("pos-process-btn");s&&(s.disabled=!0,s.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i>Memproses...');try{const d=be(),l={txId:d,date:pe.firestore.FieldValue.serverTimestamp(),dateMs:Date.now(),cashier:window.__currentAdminUid||"admin",cashierName:h.store?.name||"Kasir",customer:{name:x.name||"Pelanggan Umum",phone:x.phone||"",isMember:x.isMember||!1,memberId:x.memberId||null},items:p.map(r=>({...r})),subtotal:q(),globalDiscount:w(M),total:f(),payment:{method:u,paid:u==="cash"?T:u==="tempo"?a:f(),change:u==="cash"?G():0,bank:o,dp:a,tempoBalance:u==="tempo"?f()-a:0},status:u==="tempo"?"tempo":"paid",notes:"",source:"pos"};await H.collection("freshmart").doc("cms_data").collection("pos_transactions").doc(d).set(l),u==="tempo"&&await H.collection("freshmart").doc("cms_data").collection("orders").doc(d).set({orderId:d,source:"pos",dateString:new Date().toISOString(),customerName:l.customer.name,customerPhone:l.customer.phone,items:p.map(r=>({id:r.id,name:r.name,price:r.price,qty:r.qty})),total:f(),payment:{method:"tempo",paid:a,tempoBalance:f()-a,tempoDueDate:0,tempoPenaltyRate:1,tempoPenaltyStopped:!1},status:"processing",isTempo:!0}),A();const b={...l};p=[],M=0,k(),C(),ue(b)}catch(d){console.error("[POS] Error:",d),g("Gagal menyimpan transaksi. Coba lagi.","error"),s&&(s.disabled=!1,s.innerHTML='<i class="fa-solid fa-check-circle mr-2"></i>Proses & Simpan')}},ue=e=>{const t=e.payment.method==="cash"?`<p class="text-sm text-slate-500">Kembalian: <span class="font-black text-emerald-600">${n(e.payment.change)}</span></p>`:e.payment.method==="tempo"?'<p class="text-sm text-amber-600 font-semibold">⚠️ Dicatat sebagai Piutang Tempo</p>':`<p class="text-sm text-slate-500">Metode: ${e.payment.method.toUpperCase()}</p>`,a=JSON.stringify(e).replace(/"/g,"&quot;");document.body.insertAdjacentHTML("beforeend",`
    <div id="pos-success-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4" style="background:rgba(0,0,0,0.5)">
      <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-sm">
        <div class="p-6 text-center">
          ...
          <p class="text-2xl font-black mb-1" style="color:var(--color-primary)">${n(e.total)}</p>
          ${t}
        </div>
    ...
    </div>`)},ie=e=>{document.getElementById("pos-success-modal")?.remove();const t=h.store?.name||"TOKO PUTRI",a=h.store?.wa||"",o=h.store?.address||"",s=new Date(e.dateMs).toLocaleString("id-ID"),d=(e.items||[]).map(b=>`<tr><td style="padding:2px 0;word-wrap:break-word">${i(b.name)}</td><td style="text-align:right;padding:2px 4px;white-space:nowrap">${b.qty}x ${n(b.price)}</td><td style="text-align:right;padding:2px 0;white-space:nowrap">${n(b.subtotal)}</td></tr>`).join(""),l=window.open("","_blank","width=420,height=720");if(!l){g("Izinkan popup untuk cetak struk","warning");return}l.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Struk POS</title>
    ...
    <table>
    <tr><td>Subtotal</td><td style="text-align:right">${n(e.subtotal)}</td></tr>
    ${(e.globalDiscount||0)>0?`<tr><td>Diskon</td><td style="text-align:right">- ${n(e.globalDiscount)}</td></tr>`:""}
    <tr class="total"><td>TOTAL</td><td style="text-align:right">${n(e.total)}</td></tr>
    ${e.payment.method==="cash"?`<tr><td>Bayar</td><td style="text-align:right">${n(e.payment.paid)}</td></tr><tr><td><b>Kembalian</b></td><td style="text-align:right"><b>${n(e.payment.change)}</b></td></tr>`:""}
    ${e.payment.method==="tempo"?`<tr><td>DP</td><td style="text-align:right">${n(e.payment.dp||0)}</td></tr><tr><td>Sisa Piutang</td><td style="text-align:right">${n(e.payment.tempoBalance||0)}</td></tr>`:""}
    ...
    </body></html>`),l.document.close()},xe=()=>{j="",P="",S("admin-content",`
    ...
    </div>`),C(),k(),me(),window.posAddToCart=B,window.posUpdateQty=Y,window.posSetQty=J,window.posSetItemDisc=W,window.posRemoveItem=X,window.posClearCart=Z,window.openPayModal=ee,window.closePayModal=A,window.setPosCustomerType=se,window.setPosPayMethod=N,window.updatePosChange=oe,window.lookupPosMember=re,window.processPOSTx=le,window.printPOSReceipt=ie,window.posSetGlobalDisc=e=>{M=w(e),k()},window.posCatFilter=e=>{P=e,C()},window.posSearchFn=e=>{j=e,C()},window.openPOSHistory=()=>R(()=>Promise.resolve().then(()=>he),void 0).then(e=>e.renderPOSHistory())},U=Object.freeze(Object.defineProperty({__proto__:null,addToCart:B,clearCart:Z,closePayModal:A,lookupPosMember:re,openPayModal:ee,printPOSReceipt:ie,processPOSTx:le,removeFromCart:X,renderPOS:xe,setItemDisc:W,setPosCustomerType:se,setPosPayMethod:N,setQty:J,updatePosChange:oe,updateQty:Y},Symbol.toStringTag,{value:"Module"})),L=e=>V(e),fe=e=>new Date(e).toLocaleString("id-ID",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"});let _=new Date().toISOString().slice(0,10),v=[],D=null;const z=()=>{c("pos-hist-list")&&S("pos-hist-list",'<div class="flex justify-center py-12"><i class="fa-solid fa-spinner fa-spin text-3xl text-slate-300"></i></div>'),D&&(D(),D=null);const t=new Date(_);t.setHours(0,0,0,0);const a=new Date(_);a.setHours(23,59,59,999),D=H.collection("freshmart").doc("cms_data").collection("pos_transactions").where("dateMs",">=",t.getTime()).where("dateMs","<=",a.getTime()).orderBy("dateMs","desc").onSnapshot(o=>{v=o.docs.map(s=>s.data()),Q()},()=>{g("Gagal memuat riwayat kasir","error"),Q()})},Q=()=>{const e=v.reduce((r,m)=>r+(m.status!=="void"&&m.total||0),0),t=v.filter(r=>r.status!=="void").length,a=v.filter(r=>r.status==="void").length,o={};v.filter(r=>r.status!=="void").forEach(r=>{const m=r.payment?.method||"other";o[m]=(o[m]||0)+(r.total||0)});const s={cash:"Tunai",qris:"QRIS",transfer:"Transfer",tempo:"Tempo"},d=Object.entries(o).map(([r,m])=>`<div class="flex justify-between text-xs"><span class="text-slate-500">${s[r]||r}</span><span class="font-bold text-slate-700 dark:text-slate-200">${L(m)}</span></div>`).join(""),l=`
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-center">
            ...
            <p class="text-base font-black text-slate-800 dark:text-white">${v.filter(r=>r.status!=="void").reduce((r,m)=>r+(m.items||[]).reduce((y,I)=>y+I.qty,0),0)}</p>
        </div>
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-center">
    ...
    ${d?`<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 mb-4 space-y-1.5">${d}</div>`:""}`,b=v.length===0?`<div class="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-600"><i class="fa-solid fa-receipt text-4xl mb-3"></i><p class="font-semibold text-sm">Belum ada transaksi</p><p class="text-xs mt-1">${_}</p></div>`:v.map(r=>{const m=r.status==="void",y={cash:"emerald",qris:"blue",transfer:"violet",tempo:"amber"}[r.payment?.method]||"slate",I={cash:"Tunai",qris:"QRIS",transfer:"Transfer",tempo:"Tempo"}[r.payment?.method]||r.payment?.method;return`<div class="bg-white dark:bg-slate-800 border ${m?"border-red-200 dark:border-red-800 opacity-60":"border-slate-200 dark:border-slate-700"} rounded-2xl p-3 space-y-2 ${m?"":"hover:shadow-sm"} transition-all">
                <div class="flex items-start justify-between gap-2">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center gap-2 flex-wrap">
                            <span class="text-[10px] font-bold text-slate-500">${i(r.txId)}</span>
                            <span class="text-[9px] font-bold px-1.5 py-0.5 rounded-full bg-${y}-100 dark:bg-${y}-900/30 text-${y}-700 dark:text-${y}-400">${I}</span>
            ...
            </div>`}).join("");S("pos-hist-rekap",l),S("pos-hist-list",b)},ne=async e=>{if(await ce(`Void transaksi ${e}?
Transaksi akan ditandai batal dan tidak dihitung dalam laporan.`,"Void Transaksi"))try{await H.collection("freshmart").doc("cms_data").collection("pos_transactions").doc(e).update({status:"void"}),g("Transaksi berhasil divoid","success")}catch{g("Gagal void transaksi","error")}},ge=()=>{S("admin-content",`

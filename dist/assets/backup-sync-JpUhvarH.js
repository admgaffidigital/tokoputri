import{d as p}from"./module-member-BivWSgSm.js";import{a3 as S,a as o,a1 as f,u as l,e as d,b as L,v as P,i as I}from"./module-print-DdyfBoO_.js";import"./vendor-firebase-core-D2OF5R23.js";import"./vendor-firebase-db-BIUZcnOd.js";let C=localStorage.getItem("tokoputri_last_sync")||new Date().toISOString(),D=!1,u={products:0,categories:0,orders:0,customers:0,cashiers:0,shifts:0};const v=e=>{if(!e)return"-";try{return new Date(e).toLocaleDateString("id-ID",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"})+" WIB"}catch{return e}},N=async()=>{d("admin-content")&&(u.products=(o.products||[]).length,u.categories=(o.categories||[]).length,L("admin-content",`
                ...
                <div class="space-y-2">
                    <div class="flex items-center gap-2.5 flex-wrap">
                        ...
                        <span class="text-[11px] text-slate-500 dark:text-slate-400 font-mono" id="sync-last-time-label">
                            <i class="fa-solid fa-clock-rotate-left mr-1"></i>${v(C)}
                        </span>
                    </div>
    ...
    `),E(),B())},E=async()=>{try{p.collection("freshmart_orders").get().then(e=>{u.orders=e.size;const t=d("stat-sync-orders");t&&(t.textContent=e.size.toLocaleString("id-ID"))}).catch(()=>{}),p.collection("freshmart").doc("cms_data").collection("customers").get().then(e=>{u.customers=e.size;const t=d("stat-sync-customers");t&&(t.textContent=e.size.toLocaleString("id-ID"))}).catch(()=>{}),p.collection("freshmart").doc("cms_data").collection("cashier_accounts").get().then(e=>{u.cashiers=e.size;const t=d("stat-sync-cashiers");t&&(t.textContent=e.size.toLocaleString("id-ID"))}).catch(()=>{}),p.collection("freshmart").doc("cms_data").collection("pos_shifts").get().then(e=>{u.shifts=e.size;const t=d("stat-sync-shifts");t&&(t.textContent=e.size.toLocaleString("id-ID"))}).catch(()=>{})}catch(e){console.warn("[SyncHub] Gagal memuat ringkasan statistik live:",e)}},B=()=>{const e=localStorage.getItem("tokoputri_safety_snapshot"),t=d("btn-safety-rollback"),r=d("safety-snapshot-badge");if(!e){t&&(t.disabled=!0),r&&(r.innerHTML="");return}try{const n=JSON.parse(e);t&&(t.disabled=!1),r&&(r.innerHTML=`
                ...
                </span>`)}catch{t&&(t.disabled=!0)}},M=async()=>{if(D)return;D=!0;const e=d("btn-force-sync"),t=d("btn-force-sync-icon");t&&t.classList.add("fa-spin"),e&&e.classList.add("opacity-80","pointer-events-none"),S("Menarik & Menyinkronkan Data Cloud...");try{const r=await p.collection("freshmart").doc("cms_data").get();if(r.exists){const x=r.data()||{};Object.assign(o,x);try{localStorage.setItem("freshmart_cms_data_cache",JSON.stringify(x))}catch{}}const n=await p.collection("freshmart").doc("cms_data").collection("customers").get();o.customers=n.docs.map(x=>x.data()),u.customers=n.size;const i=await p.collection("freshmart_orders").orderBy("timestamp","desc").limit(100).get();u.orders=i.size,C=new Date().toISOString(),localStorage.setItem("tokoputri_last_sync",C),u.products=(o.products||[]).length,u.categories=(o.categories||[]).length;const s=d("stat-sync-products");s&&(s.textContent=u.products);const a=d("stat-sync-categories");a&&(a.textContent=u.categories);const k=d("stat-sync-orders");k&&(k.textContent=u.orders.toLocaleString("id-ID"));const g=d("stat-sync-customers");g&&(g.textContent=u.customers.toLocaleString("id-ID"));const b=d("sync-last-time-label");b&&(b.innerHTML=`<i class="fa-solid fa-clock-rotate-left mr-1"></i>${v(C)}`),l("Semua data toko berhasil disinkronkan langsung dari Cloud Firestore!","success")}catch(r){console.error("[SyncHub] Gagal menarik pembaruan cloud:",r),l("Gagal menyinkronkan data cloud. Periksa koneksi internet Anda.","error")}finally{D=!1,f(),t&&t.classList.remove("fa-spin"),e&&e.classList.remove("opacity-80","pointer-events-none")}},F=async()=>{S("Mengumpulkan seluruh data ekosistem toko...");try{const[e,t,r,n,i]=await Promise.all([p.collection("freshmart_orders").get().catch(()=>({docs:[]})),p.collection("freshmart").doc("cms_data").collection("customers").get().catch(()=>({docs:[]})),p.collection("freshmart").doc("cms_data").collection("cashier_accounts").get().catch(()=>({docs:[]})),p.collection("freshmart").doc("cms_data").collection("pos_shifts").get().catch(()=>({docs:[]})),p.collection("freshmart").doc("cms_data").collection("reviews").get().catch(()=>({docs:[]}))]),s=e.docs.map(c=>({_id:c.id,...c.data()})),a=t.docs.map(c=>({_id:c.id,...c.data()})),k=r.docs.map(c=>({_id:c.id,...c.data()})),g=n.docs.map(c=>({_id:c.id,...c.data()})),b=i.docs.map(c=>({_id:c.id,...c.data()})),x=new Date,h=x.toISOString().slice(0,10),w=`${String(x.getHours()).padStart(2,"0")}-${String(x.getMinutes()).padStart(2,"0")}`,_={_meta:{appName:"Toko Putri Super App",version:"1.9.36",backupType:"full_ecosystem",createdAt:x.toISOString(),exportedBy:window.isAdm?"Seller Admin":"Staff",stats:{productsCount:(o.products||[]).length,categoriesCount:(o.categories||[]).length,ordersCount:s.length,customersCount:a.length,cashiersCount:k.length,shiftsCount:g.length},checksum:btoa(`${h}-${o.products?.length||0}-${s.length}`).slice(0,16)},appData:{store:o.store||{},products:o.products||[],categories:o.categories||[],brands:o.brands||[],colors:o.colors||[],banners:o.banners||[],vouchers:o.vouchers||[],banks:o.banks||[],faqs:o.faqs||[],tax:o.tax||{},rewards:o.rewards||[],hasCashier:o.hasCashier||!1},orders:s,customers:a,cashiers:k,shifts:g,reviews:b},A=JSON.stringify(_,null,2),T=`backup_tokoputri_full_${h}_${w}.json`;if(window.AndroidNativeApp&&typeof window.AndroidNativeApp.saveOrShareFile=="function"){const c=btoa(unescape(encodeURIComponent(A)));window.AndroidNativeApp.saveOrShareFile(c,T,"application/json")}else{const c=new Blob([A],{type:"application/json;charset=utf-8;"}),j=URL.createObjectURL(c),y=document.createElement("a");y.href=j,y.download=T,document.body.appendChild(y),y.click(),y.remove(),setTimeout(()=>URL.revokeObjectURL(j),1e3)}f(),l(`Cadangan lengkap berhasil diunduh (${s.length} transaksi, ${o.products?.length||0} produk)!`,"success")}catch(e){f(),console.error("[BackupEngine] Gagal mengekspor data komprehensif:",e),l("Gagal membuat cadangan database toko: "+e.message,"error")}},m=e=>e==null?'""':`"${String(e).replace(/"/g,'""')}"`,$=()=>{try{const e=o.products||[];if(e.length===0){l("Belum ada data produk untuk diekspor!","warning");return}const t=["ID Produk","Nama Produk","Barcode / SKU","Kategori","Merek","Harga Modal (HPP Rp)","Harga Jual Toko (Rp)","Harga Grosir (Rp)","Minimal Grosir","Stok Saat Ini","Satuan","Deskripsi Singkat"],r=e.map(s=>[m(s.id||""),m(s.name||""),m(s.sku||s.barcode||""),m(s.category||"Umum"),m(s.brand||"-"),s.hpp||0,s.price||0,s.wholesalePrice||0,s.wholesaleMin||0,s.stock??0,m(s.unit||"pcs"),m((s.desc||"").replace(/(\r\n|\n|\r)/gm," "))]),n="\uFEFF"+[t.join(","),...r.map(s=>s.join(","))].join(`\r
`),i=`laporan_produk_tokoputri_${new Date().toISOString().slice(0,10)}.csv`;O(n,i),l(`Berhasil mengekspor ${e.length} produk ke CSV!`,"success")}catch(e){console.error("[CSVExport] Gagal ekspor produk:",e),l("Gagal mengekspor CSV produk","error")}},U=async()=>{S("Menyiapkan laporan transaksi CSV...");try{const e=await p.collection("freshmart_orders").orderBy("timestamp","desc").get();if(e.empty){f(),l("Belum ada data pesanan/transaksi untuk diekspor!","warning");return}const t=e.docs.map(a=>({id:a.id,...a.data()})),r=["No Invoice","Waktu Transaksi","Sumber Transaksi","Nama Kasir / Staf","Nama Pelanggan","No WhatsApp","Total Item","Subtotal (Rp)","Diskon (Rp)","Pajak / PPN (Rp)","Total Bayar (Rp)","Metode Pembayaran","Status Pesanan"],n=t.map(a=>{const k=a.timestamp?v(a.timestamp):"-",g=a.source==="pos"?"Kasir POS":"Pesanan Web",b=a.cashierName||(a.source==="pos"?"Kasir Toko":"Website"),x=a.customerName||(a.customer?a.customer.name:"Pelanggan Toko"),h=a.customerPhone||(a.customer?a.customer.phone:"-"),w=(a.items||[]).reduce((_,A)=>_+(A.qty||1),0);return[m(a.id||""),m(k),m(g),m(b),m(x),m(h),w,a.subtotal||a.total||0,a.discount||0,a.tax||0,a.total||0,m(a.paymentMethod||"Tunai"),m(a.status||"Selesai")]}),i="\uFEFF"+[r.join(","),...n.map(a=>a.join(","))].join(`\r
`),s=`laporan_transaksi_tokoputri_${new Date().toISOString().slice(0,10)}.csv`;O(i,s),f(),l(`Berhasil mengekspor ${t.length} data transaksi ke CSV!`,"success")}catch(e){f(),console.error("[CSVExport] Gagal ekspor pesanan:",e),l("Gagal mengekspor data transaksi: "+e.message,"error")}},O=(e,t)=>{if(window.AndroidNativeApp&&typeof window.AndroidNativeApp.saveOrShareFile=="function"){const r=btoa(unescape(encodeURIComponent(e)));window.AndroidNativeApp.saveOrShareFile(r,t,"text/csv")}else{const r=new Blob([e],{type:"text/csv;charset=utf-8;"}),n=URL.createObjectURL(r),i=document.createElement("a");i.href=n,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(n),1e3)}},V=e=>{const t=e.target.files?.[0];if(!t)return;const r=new FileReader;r.onload=async n=>{try{const i=n.target.result,s=JSON.parse(i),a=!!(s._meta&&s.appData),k=!!(s.products&&Array.isArray(s.products));if(!a&&!k){l("Format berkas tidak dikenali! Pastikan berkas cadangan resmi Toko Putri (.json).","error");return}const b=((a?s.appData:s).products||[]).length,x=a&&s.orders?s.orders.length:0,h=a&&s.customers?s.customers.length:0,w=a&&s._meta?.createdAt?v(s._meta.createdAt):t.name;K({fileName:t.name,backupDate:w,productsCount:b,ordersCount:x,custCount:h,isFullBackup:a,backupDataPayload:s})}catch(i){console.error("[RestoreEngine] Gagal membaca berkas cadangan:",i),l("Berkas JSON rusak atau tidak dapat dibaca!","error")}finally{e.target.value=""}},r.readAsText(t)},K=({fileName:e,backupDate:t,productsCount:r,ordersCount:n,custCount:i,isFullBackup:s,backupDataPayload:a})=>{const k=d("modal-pre-restore-inspector");k&&k.remove();const g=`
    ...
    `;document.body.insertAdjacentHTML("beforeend",g);const b=d("btn-execute-restore");b&&(b.onclick=()=>{d("modal-pre-restore-inspector")?.remove(),R(a)})},R=async e=>{S("Membuat Safety Snapshot & Memulihkan Database...");try{const t={timestamp:new Date().toISOString(),appData:JSON.parse(JSON.stringify(o))};localStorage.setItem("tokoputri_safety_snapshot",JSON.stringify(t));const r=!!(e._meta&&e.appData),n=r?e.appData:e;if(Object.assign(o,n),typeof window.saveApp=="function"?await window.saveApp():await p.collection("freshmart").doc("cms_data").set(o,{merge:!0}),r&&e.customers&&Array.isArray(e.customers)){const i=p.batch(),s=p.collection("freshmart").doc("cms_data").collection("customers");e.customers.slice(0,100).forEach(a=>{if(a.phone||a._id){const k=String(a.phone||a._id);i.set(s.doc(k),a,{merge:!0})}}),await i.commit().catch(()=>{})}f(),l("Database toko berhasil dipulihkan secara akurat!","success"),setTimeout(()=>{N()},600)}catch(t){f(),console.error("[RestoreEngine] Gagal mengeksekusi restore:",t),l("Gagal memulihkan database toko: "+t.message,"error")}},G=async()=>{const e=localStorage.getItem("tokoputri_safety_snapshot");if(!e){l("Tidak ada riwayat snapshot keselamatan yang tersimpan!","warning");return}let t;try{t=JSON.parse(e)}catch{l("Snapshot keselamatan rusak!","error");return}if(await P("Batalkan & Rollback Data?",`Apakah Anda yakin ingin membatalkan pemulihan dan mengembalikan data toko ke snapshot tanggal ${v(t.timestamp)}?`)){S("Mengembalikan data ke kondisi semula (Rollback)...");try{Object.assign(o,t.appData),typeof window.saveApp=="function"?await window.saveApp():await p.collection("freshmart").doc("cms_data").set(o,{merge:!0}),localStorage.removeItem("tokoputri_safety_snapshot"),f(),l("Data toko telah berhasil dikembalikan ke kondisi semula!","success"),setTimeout(()=>{N()},600)}catch(n){f(),console.error("[RollbackEngine] Gagal rollback:",n),l("Gagal mengembalikan data: "+n.message,"error")}}},H=()=>{try{const e={timestamp:new Date().toISOString(),appData:JSON.parse(JSON.stringify(o))};localStorage.setItem("tokoputri_quick_snapshot",JSON.stringify(e)),l("Snapshot cepat berhasil disimpan di memori perangkat!","success")}catch{l("Memori lokal penuh, gagal menyimpan snapshot!","error")}},J=async()=>{const e=localStorage.getItem("tokoputri_quick_snapshot");if(!e){l("Belum ada snapshot cepat yang disimpan di perangkat ini!","warning");return}let t;try{t=JSON.parse(e)}catch{l("Snapshot kilat rusak!","error");return}await P("Terapkan Snapshot Cepat?",`Pulihkan data toko ke snapshot yang disimpan pada ${v(t.timestamp)}?`)&&R(t.appData)};window.renderBackupSyncView=N;window.triggerRealtimeSync=M;window.downloadFullBackupJSON=F;window.exportProductsCSV=$;window.exportOrdersCSV=U;window.handleRestoreFileSelect=V;window.triggerSafetyRollback=G;window.saveQuickDeviceSnapshot=H;window.restoreQuickDeviceSnapshot=J;export{F as downloadFullBackupJSON,U as exportOrdersCSV,$ as exportProductsCSV,V as handleRestoreFileSelect,N as renderBackupSyncView,J as restoreQuickDeviceSnapshot,H as saveQuickDeviceSnapshot,M as triggerRealtimeSync,G as triggerSafetyRollback};

...
Kami akan segera memproses pesanan Anda. Terima kasih! 🙏`;window.open(`https://wa.me/${r}?text=${encodeURIComponent(p)}`,"_blank")}catch{P(),u("Gagal memuat data pesanan!")}},es=e=>{ve("Hapus Pesanan","Yakin ingin hapus permanen?",async()=>{if(!Ve){me(!0),C("Menghapus...");try{await k.collection("freshmart_orders").doc(e).delete(),u("Terhapus!"),ca===e&&Vt()}catch{u("Gagal!")}finally{me(!1),P()}}})};window.exportOrdersToExcel=Wa;window.rAdmOrd=Ja;window.openOrderDetail=mt;window.saveOrderCustomerToDB=Qa;window.ackRewardClaim=Ya;window.closeOrderDetailModal=Vt;window.updateOrderStatus=Xa;window.konfirmasiKeWA=Za;window.deleteOrder=es;const ts=()=>{const e=o.store.name||"Toko Grosir",t=o.store.themeColor||"#10b981",a=(l,n,i=!1)=>{let d=document.querySelector(`meta[${i?"property":"name"}="${l}"]`);d||(d=document.createElement("meta"),i?d.setAttribute("property",l):d.setAttribute("name",l),document.head.appendChild(d)),d.setAttribute("content",n)};a("theme-color",t),a("mobile-web-app-capable","yes"),a("apple-mobile-web-app-capable","yes"),a("apple-mobile-web-app-status-bar-style","black-translucent"),a("apple-mobile-web-app-title",e),a("application-name",e),a("msapplication-TileColor",t),document.title=e,localStorage.setItem("freshmart_theme_color",t),o.store.uiTheme&&o.store.uiTheme!==localStorage.getItem("freshmart_ui_theme")&&(localStorage.setItem("freshmart_ui_theme",o.store.uiTheme),Pe(o.store.uiTheme));const r=o.store.bgStyle||localStorage.getItem("freshmart_bg_style")||"minimalist",s=o.store.bgCustomUrl!==void 0?o.store.bgCustomUrl:localStorage.getItem("freshmart_bg_custom_url")||"";Ae(r,s)},Gt=()=>{$("admin-content",`
    ...
    `)},xt=()=>{const e=Re(),t=R===0?`Tahun ${_}`:`${re[R-1]} ${_}`,a=e.omset-e.disc-e.hpp,r=R===0?null:`${_}-${R}`,s=ft(),l=a-s,n=o.taxSettings?.taxScheme||"umkm_final";let i,d,p;n==="umkm_final"?(i=.5,d=e.omset,p="PPh Final UMKM (0,5% × Omset)"):n==="badan_normal"?(i=22,d=Math.max(0,l),p="PPh Badan (22% × Laba Bersih)"):(i=parseFloat(o.taxSettings?.customTaxRate)||0,d=Math.max(0,l),p=`PPh Custom (${i}% × Laba Bersih)`);const m=d*(i/100),b=l-m;let g="";if(R===0)g=Array.from({length:12},(v,f)=>f+1).map(v=>{const f=`${_}-${v}`,S=(o.taxSettings?.monthlyExpenses||{})[f]||0;return`<div class="flex items-center justify-between gap-2 py-2 border-b border-slate-100 dark:border-slate-700/50 last:border-0">
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-[11px] font-bold text-amber-700 leading-relaxed">
    ...
    </div>`,d="";if(e==="summary"){const m=Re(),b=m.omset-m.disc,g=Array.from({length:12},(v,f)=>f+1).map(v=>{const f=se?se[v]:{omset:0,ppn:0,orderCount:0};return`<tr class="border-b border-slate-200"><td class="py-2.5 px-3 font-bold text-slate-700">${re[v-1]} ${_}</td><td class="py-2.5 px-3 text-right font-bold text-slate-700">${x(f.omset)}</td><td class="py-2.5 px-3 text-right font-bold text-slate-900">${x(f.ppn)}</td><td class="py-2.5 px-3 text-right font-bold text-slate-500">${f.orderCount}</td></tr>`}).join("");d=`
        ...
        <table class="w-full text-xs"><thead><tr class="bg-slate-100 text-left"><th class="py-2.5 px-3 font-bold text-slate-500 uppercase text-[9px]">Bulan</th><th class="py-2.5 px-3 font-bold text-slate-500 uppercase text-[9px] text-right">Omset</th><th class="py-2.5 px-3 font-bold text-slate-500 uppercase text-[9px] text-right">PPN Keluaran</th><th class="py-2.5 px-3 font-bold text-slate-500 uppercase text-[9px] text-right">Pesanan</th></tr></thead><tbody>${g}</tbody></table>`}else if(e==="income"){const m=Re(),b=m.omset-m.disc-m.hpp,g=ft(),v=b-g,f=a.taxScheme||"umkm_final";let S,T,A;f==="umkm_final"?(S=.5,T=m.omset,A="PPh Final UMKM (0,5% × Omset)"):f==="badan_normal"?(S=22,T=Math.max(0,v),A="PPh Badan (22% × Laba Bersih)"):(S=parseFloat(a.customTaxRate)||0,T=Math.max(0,v),A=`PPh Custom (${S}% × Laba Bersih)`);const w=T*(S/100),V=v-w,B=(D,M,E,Ye)=>`<div class="flex justify-between py-2 ${E?"border-t-2 border-slate-800 mt-1 pt-3":"border-b border-slate-100"}"><span class="${E?"font-bold text-slate-900":"font-bold text-slate-600"}">${D}</span><span class="font-bold ${Ye||"text-slate-900"}">${M}</span></div>`;d=`<div class="max-w-xl">
            ...
            ${B("Laba Bersih Sebelum Pajak",x(v),!0)}
    ...
    `},Bs=()=>{let e=document.getElementById("guarantee-modal");e||(e=document.createElement("div"),e.id="guarantee-modal",e.className="fixed inset-0 z-[115] bg-slate-900/80 flex items-end sm:items-center justify-center p-0 sm:p-5",e.onclick=t=>{t.target===e&&Qt()},document.body.appendChild(e)),e.innerHTML=`
...
`).map(i=>i.replace(/^[-*•]\s*/,"").trim()).filter(i=>i.length>0);if(l.length===0)return u("Rincian perubahan tidak boleh kosong!");C("Menyimpan catatan pembaruan...");const n={id:ye||"log-"+Date.now().toString(36),version:e.startsWith("v")?e:"v"+e,category:t,date:a,title:r,items:l,updatedAt:new Date().toISOString()};if(o.changelog=o.changelog||[],ye){const i=o.changelog.findIndex(d=>d.id===ye);i!==-1?o.changelog[i]=n:o.changelog.unshift(n)}else o.changelog.unshift(n);try{await K(["changelog"]),u("Catatan pembaruan berhasil dipublikasikan secara real-time!","success"),$e=!1,ye=null,Fe()}catch(i){u("Gagal menyimpan log pembaruan: "+i.message,"error")}finally{P()}},ar=e=>{const t=(o.changelog||[]).find(a=>a.id===e);t&&ve(`Hapus catatan pembaruan versi "${t.version}"?`,async()=>{C("Menghapus catatan..."),o.changelog=(o.changelog||[]).filter(a=>a.id!==e);try{await K(["changelog"]),u("Catatan berhasil dihapus!","success"),Fe()}catch(a){u("Gagal menghapus catatan: "+a.message,"error")}finally{P()}},"Ya, Hapus","Konfirmasi Hapus")};window.rAdmChangelog=Fe;window.toggleChangelogForm=Zs;window.editChangelogEntry=er;window.saveChangelogEntry=tr;window.deleteChangelogEntry=ar;export{Sa as A,Se as a,Pe as b,Ae as c,k as d,Es as e,_s as f,Rs as g,$a as h,lr as i,Ns as j,q as k,ir as l,Ot as m,Ht as n,qe as o,Ge as p,Nt as q,Ee as r,ts as s,nr as t,Y as u,dr as v,Jt as w};

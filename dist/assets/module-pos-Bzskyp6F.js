            ...
            <div class="group flex items-center gap-2.5 p-2 sm:p-2.5 bg-white dark:bg-slate-800/90 rounded-2xl border border-slate-200/90 dark:border-slate-700/80 shadow-xs hover:border-[var(--color-primary)] transition-all">
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        ${i.isWholesale?'<span class="inline-flex items-center text-[8px] font-black px-1 rounded bg-amber-500 text-white shadow-2xs">GROSIR</span>':""}
                        ${i.isVariant?'<span class="inline-flex items-center text-[8px] font-black px-1 rounded bg-indigo-600 text-white shadow-2xs">VARIAN</span>':""}
                        <span class="text-[10px] text-slate-500 font-medium">
                            ${i.isWholesale&&i.basePrice?`<span class="line-through text-slate-400">${x(i.basePrice)}</span> <span class="text-amber-600 font-bold">${x(i.price)}</span>`:x(i.price)}
                        </span>
                    </div>

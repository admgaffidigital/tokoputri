...
Kami akan segera memproses pesanan Anda. Terima kasih! 🙏`;window.open(`https://wa.me/${o}?text=${encodeURIComponent(d)}`,"_blank")}catch{P(),u("Gagal memuat data pesanan!")}},Oa=e=>{he("Hapus Pesanan","Yakin ingin hapus permanen?",async()=>{if(!He){pe(!0),T("Menghapus...");try{await y.collection("freshmart_orders").doc(e).delete(),u("Terhapus!"),Xt===e&&Mt()}catch{u("Gagal!")}finally{pe(!1),P()}}})};window.exportOrdersToExcel=La;window.rAdmOrd=_a;window.openOrderDetail=rt;window.saveOrderCustomerToDB=Ea;window.ackRewardClaim=Na;window.closeOrderDetailModal=Mt;window.updateOrderStatus=Ra;window.konfirmasiKeWA=Fa;window.deleteOrder=Oa;const Ha=()=>{const e=r.store.name||"Toko Grosir",t=r.store.themeColor||"#10b981",a=(i,n,l=!1)=>{let p=document.querySelector(`meta[${l?"property":"name"}="${i}"]`);p||(p=document.createElement("meta"),l?p.setAttribute("property",i):p.setAttribute("name",i),document.head.appendChild(p)),p.setAttribute("content",n)};a("theme-color",t),a("mobile-web-app-capable","yes"),a("apple-mobile-web-app-capable","yes"),a("apple-mobile-web-app-status-bar-style","black-translucent"),a("apple-mobile-web-app-title",e),a("application-name",e),a("msapplication-TileColor",t),document.title=e,localStorage.setItem("freshmart_theme_color",t),r.store.uiTheme&&r.store.uiTheme!==localStorage.getItem("freshmart_ui_theme")&&(localStorage.setItem("freshmart_ui_theme",r.store.uiTheme),$e(r.store.uiTheme));const o=r.store.bgStyle||localStorage.getItem("freshmart_bg_style")||"minimalist",s=r.store.bgCustomUrl!==void 0?r.store.bgCustomUrl:localStorage.getItem("freshmart_bg_custom_url")||"";Se(o,s)},Lt=()=>{S("admin-content",`
    ...
    <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6 text-[11px] font-bold text-amber-700 leading-relaxed">
        ...
        </div>`}else if(e==="balance"){const m=Ue(),b=a.balanceSheet||{kas:0,piutang:0,hutang:0},f=(parseFloat(b.kas)||0)+(parseFloat(b.piutang)||0)+m.assetHpp,w=parseFloat(b.hutang)||0,x=f-w;p=`
    ...
    `;e.length===0?a+=`<div class="bg-white dark:bg-slate-800 p-8 text-center rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            </div>`}),a+="</div>"),a+="</div>",S("admin-content",a),setTimeout(()=>{h("total-piutang-header")&&(h("total-piutang-header").innerText=g(t))},100)};const os=e=>{ea(e),Et()},Et=()=>{const e=Zt||"all",t=(Ze||[]).filter(i=>e==="visible"?i.isVisible!==!1:e==="hidden"?i.isVisible===!1:!0),a=`

        ...
        <div class="p-4 sm:p-5 md:p-6 lg:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-4 rounded-[1.5rem] border shadow-sm cursor-pointer hover:shadow-md hover:-translate-y-1 hover:border-[var(--color-primary)]/40 dark:hover:border-[var(--color-primary)]/40 transition-all duration-300 ${m}" onclick="oAEd('${e}','${i.id}')">
            <div class="flex items-start sm:items-center gap-4 min-w-0 w-full">
                ...
                <div class="min-w-0 flex flex-col justify-center py-1">
                    ...
                    ${d&&h&&A?`<p class="text-[10px] font-bold mt-1 ${(i.variants&&i.variants.length?i.variants.reduce((D,I)=>D+(parseFloat(I.stock)||0),0):parseFloat(i.stock)||0)===0?"text-rose-500 animate-pulse":"text-blue-500"}"><i class="fa-solid fa-boxes-stacked mr-1"></i>Stok: ${i.variants&&i.variants.length?i.variants.reduce((D,I)=>D+(parseFloat(I.stock)||0),0).toFixed(2).replace(/\.?0+$/,""):parseFloat(i.stock)||0}</p>`:""}
                    ${d&&h&&i.hpp?`<p class="text-[10px] font-bold text-amber-500 mt-0.5"><i class="fa-solid fa-coins mr-1"></i>HPP: ${x(i.hpp)}</p>`:""}
                    ${d?(()=>{const D=i.variants&&i.variants.length?i.variants.reduce((I,E)=>I+(parseFloat(E.totalSold)||0),0):parseFloat(i.totalSold)||0;return D>0?`<p class="text-[10px] font-bold text-orange-400 mt-0.5"><i class="fa-solid fa-fire-flame-curved mr-1"></i>Terjual: ${D}</p>`:""})():""}

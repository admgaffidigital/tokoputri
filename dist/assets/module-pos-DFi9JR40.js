...
import{a as ee,A as ca,d as $,_ as pa}from"./module-member-Bpdjsr56.js";import{e as i,t as v,a as u,i as d,ar as Y,x as Be,b as ba,f as ua}from"./module-print-C2-MjUR_.js";import{f as Re}from"./vendor-firebase-core-D2OF5R23.js";let ie=null;const Q=()=>{if(ie)return ie;try{const e=sessionStorage.getItem("pos_cashier_session");if(e)return ie=JSON.parse(e),ie}catch{}return null},xa=e=>{ie=e;try{e?sessionStorage.setItem("pos_cashier_session",JSON.stringify(e)):sessionStorage.removeItem("pos_cashier_session")}catch{}},Tt=()=>{ie=null;try{sessionStorage.removeItem("pos_cashier_session")}catch{}},$t=()=>!!Q(),Ct=async()=>{if(Q()||window.isAdm||window.__localIsAdm||u&&(u.hasCashier===!0||u.store?.posEnabled===!0))return!0;const e=localStorage.getItem("pos_has_cashier");if(e==="true")return!0;const t=!!(window.isAdm||window.__localIsAdm||ee.currentUser&&ee.currentUser.uid===ca);try{if(t){const s=!(await $.collection("freshmart").doc("cms_data").collection("cashier_accounts").where("isActive","==",!0).limit(1).get()).empty;try{localStorage.setItem("pos_has_cashier",s?"true":"false"),$.collection("freshmart").doc("cms_data").set({hasCashier:s},{merge:!0}).catch(()=>{})}catch{}return s}else{const a=await $.collection("freshmart").doc("cms_data").get();if(a.exists){const s=a.data();if(s.hasCashier!==void 0){const r=!!s.hasCashier;try{localStorage.setItem("pos_has_cashier",r?"true":"false")}catch{}return r}}return e!=="false"}}catch{return e!=="false"}},Me=async()=>{const e=i("pos-cashier-header-btn");if(!e)return;const t=!!Q(),a=!!(window.isAdm||window.__localIsAdm),s=localStorage.getItem("pos_has_cashier"),r=u?u.hasCashier??!0:!0;t||a||s==="true"||s===null&&r!==!1?e.classList.remove("hidden"):s==="false"&&e.classList.add("hidden");try{await Ct()||t||a?e.classList.remove("hidden"):e.classList.add("hidden")}catch{(t||a||s!=="false")&&e.classList.remove("hidden")}},Mt=async()=>{typeof window.triggerHaptic=="function"&&window.triggerHaptic("medium"),Q()?(typeof window.changeView=="function"&&window.changeView("view-pos-cashier"),typeof window.renderPOSStorefront=="function"&&window.renderPOSStorefront()):Ke()},Ke=()=>{const e=i("pos-login-modal");e&&(e.classList.remove("hidden"),setTimeout(()=>{e.classList.remove("opacity-0");const t=i("pos-login-modal-box");t&&t.classList.remove("translate-y-full","scale-95")},10),setTimeout(()=>{const t=i("pos-login-email");t&&t.focus()},300),typeof window.triggerHaptic=="function"&&window.triggerHaptic("light"))},_e=()=>{const e=i("pos-login-modal"),t=i("pos-login-modal-box");e&&e.classList.add("opacity-0"),t&&t.classList.add("translate-y-full"),setTimeout(()=>{e&&e.classList.add("hidden");const a=i("pos-login-email"),s=i("pos-login-password"),r=i("pos-login-error");a&&(a.value=""),s&&(s.value=""),r&&(r.textContent="",r.classList.add("hidden"))},300)},At=async()=>{const e=i("pos-login-email"),t=i("pos-login-password"),a=i("pos-login-error"),s=i("pos-login-btn"),r=e?.value?.trim()||"",n=t?.value||"",o=p=>{if(a){a.classList.remove("hidden");const b=a.querySelector("span");b?b.textContent=p:a.textContent=p}typeof window.triggerHaptic=="function"&&window.triggerHaptic("error")};if((()=>{if(a){a.classList.add("hidden");const p=a.querySelector("span");p&&(p.textContent="")}})(),!r||!n){o("Email dan password wajib diisi.");return}s&&(s.disabled=!0,s.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i>Memverifikasi...');try{const b=(await ee.signInWithEmailAndPassword(r,n)).user?.uid;if(!b)throw new Error("UID tidak ditemukan");const y=await $.collection("freshmart").doc("cms_data").collection("cashier_accounts").doc(b).get();if(!y.exists){await ee.signOut(),o("Akun ini bukan akun kasir yang terdaftar di toko ini.");return}const k=y.data();if(k.role!=="cashier"){await ee.signOut(),o("Akun ini tidak memiliki akses kasir.");return}if(!k.isActive){await ee.signOut(),o("Akun kasir ini telah dinonaktifkan. Hubungi admin toko.");return}xa({uid:b,name:k.name||r,email:k.email||r,role:"cashier"});try{localStorage.setItem("pos_has_cashier","true")}catch{}Me(),_e(),v(`Selamat datang, ${k.name||"Kasir"}! 👋`,"success"),typeof window.changeView=="function"&&window.changeView("view-pos-cashier"),setTimeout(()=>{typeof window.renderPOSStorefront=="function"&&window.renderPOSStorefront()},100),typeof window.triggerHaptic=="function"&&window.triggerHaptic("success")}catch(p){console.error("[POS Auth] Login error:",p);const b=p.code||"";o(b==="auth/user-not-found"||b==="auth/wrong-password"||b==="auth/invalid-credential"?"Email atau password salah.":b==="auth/too-many-requests"?"Terlalu banyak percobaan. Coba lagi beberapa saat.":b==="auth/network-request-failed"?"Koneksi gagal. Periksa jaringan internet.":"Login gagal: "+(p.message||"Kesalahan tidak diketahui"))}finally{s&&(s.disabled=!1,s.innerHTML='<i class="fa-solid fa-right-to-bracket mr-2"></i>Masuk Kasir')}},qe=async(e=!1)=>{if(!e&&typeof window.getActiveShift=="function"){const a=window.getActiveShift();if(a&&a.status==="open"){document.getElementById("pos-logout-shift-modal")?.remove();const s=typeof window.fRp=="function"?window.fRp(a.startingCash):"Rp "+a.startingCash;document.body.insertAdjacentHTML("beforeend",`
            ...
            </div>`);return}}Q(),typeof window.detachPOSHistoryListener=="function"&&window.detachPOSHistoryListener();try{if(!window.isAdm&&!window.__localIsAdm)try{await ee.signOut()}catch{}}catch{}Tt();const t=i("view-pos-cashier");t&&(t.innerHTML=""),typeof window.destroyBarcodeListener=="function"&&window.destroyBarcodeListener(),typeof window.stopPOSClock=="function"&&window.stopPOSClock(),v("Sesi kasir berakhir. Sampai jumpa! 👋"),typeof window.changeView=="function"&&window.changeView("view-catalog"),typeof window.triggerHaptic=="function"&&window.triggerHaptic("medium"),typeof window.updatePOSHeaderIcon=="function"&&window.updatePOSHeaderIcon()},Ot=async()=>{await Me()};window.openPOSCashierMode=Mt;window.openPOSLoginModal=Ke;window.closePOSLoginModal=_e;window.processCashierLogin=At;window.cashierLogout=qe;window.exitPOSMode=qe;window.getCashierSession=Q;window.isCashierLoggedIn=$t;window.initPOSAuth=Ot;window.updatePOSHeaderIcon=Me;const Ia=Object.freeze(Object.defineProperty({__proto__:null,cashierLogout:qe,checkCashierExists:Ct,clearCashierSession:Tt,closePOSLoginModal:_e,getCashierSession:Q,initPOSAuth:Ot,isCashierLoggedIn:$t,openPOSCashierMode:Mt,openPOSLoginModal:Ke,processCashierLogin:At,updatePOSHeaderIcon:Me},Symbol.toStringTag,{value:"Module"})),m=e=>"Rp "+Math.round(parseFloat(e)||0).toLocaleString("id-ID"),Te="pos_active_shift",Lt="pos_last_closed_shift";let le=null;const B=()=>{if(le)return le;try{const e=localStorage.getItem(Te);if(e)return le=JSON.parse(e),le}catch(e){console.warn("[POS Shift] Gagal membaca active shift:",e)}return null},Ve=e=>{le=e;try{e?localStorage.setItem(Te,JSON.stringify(e)):localStorage.removeItem(Te)}catch{}},jt=()=>{le=null;try{localStorage.removeItem(Te)}catch{}},ma=()=>{try{const e=localStorage.getItem(Lt);if(e)return JSON.parse(e)}catch{}return null},fa=e=>{try{localStorage.setItem(Lt,JSON.stringify(e))}catch{}},pe=()=>{const e=B();return!!(e&&e.status==="open")},It=(e="open")=>{try{const t=window.AudioContext||window.webkitAudioContext;if(!t)return;const a=new t,s=a.currentTime;e==="open"?([523.25,659.25,783.99,1046.5].forEach((n,o)=>{const c=a.createOscillator(),p=a.createGain(),b=s+o*.07;c.type="sine",c.frequency.setValueAtTime(n,b),p.gain.setValueAtTime(.09,b),p.gain.exponentialRampToValueAtTime(1e-4,b+.16),c.connect(p),p.connect(a.destination),c.start(b),c.stop(b+.16)}),setTimeout(()=>{a.close().catch(()=>{})},600)):([{f:[783.99,987.77],t:s,d:.14},{f:[1046.5,1318.51],t:s+.12,d:.35}].forEach(n=>{n.f.forEach(o=>{const c=a.createOscillator(),p=a.createGain();c.type="triangle",c.frequency.setValueAtTime(o,n.t),p.gain.setValueAtTime(.08,n.t),p.gain.exponentialRampToValueAtTime(1e-4,n.t+n.d),c.connect(p),p.connect(a.destination),c.start(n.t),c.stop(n.t+n.d)})}),setTimeout(()=>{a.close().catch(()=>{})},700))}catch{}},Ue=(e,t=Date.now())=>{if(!e)return"-";const a=Math.max(0,t-e),s=Math.floor(a/6e4),r=Math.floor(s/60),n=s%60;return r>0?`${r} Jam ${n} Menit`:`${n} Menit`},ga=()=>{const e=new Date,t=e.getFullYear(),a=String(e.getMonth()+1).padStart(2,"0"),s=String(e.getDate()).padStart(2,"0"),r=Math.floor(100+Math.random()*900);return`SHF-${t}${a}${s}-${r}`},oe=()=>{const t=Q()?.name||(window.isAdm?"Admin Seller":"Kasir Toko"),a=new Date().toLocaleString("id-ID",{dateStyle:"full",timeStyle:"short"});document.getElementById("pos-open-shift-modal")?.remove();const s=`
                ...
                <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex items-center justify-between">
                    <div>
                        ...
                        <div class="text-xs text-slate-600 dark:text-slate-300 mt-0.5">
                            Modal Awal: <b>${m(e.startingCash)}</b> + Penjualan Tunai: <b>${m(e.cashSales||0)}</b>
                        </div>
                    </div>
                    <div class="text-right">
    ...
    <div id="pos-closed-success-modal" class="fixed inset-0 z-[10002] flex items-center justify-center p-3 sm:p-4" style="background:rgba(15,23,42,0.8);backdrop-filter:blur(6px)">
    ...
    </div>`;document.body.insertAdjacentHTML("beforeend",s)},Qe=(e,t=!1)=>{if(!e){v("Data shift tidak ditemukan.","warning");return}const a=typeof Y=="function"?Y():{paperSize:"58mm"},s=a.paperSize==="80mm",r=a.headerText||u.store?.name||"TOKO PUTRI",n=u.store?.address||"",o=u.store?.wa||"",c=a.footerText||"Laporan Kasir Resmi Toko Putri",p=t?"RINGKASAN SHIFT (X-REPORT)":"REKAP TUTUP SHIFT (Z-REPORT)",b=new Date(e.startTime).toLocaleString("id-ID",{dateStyle:"short",timeStyle:"short"}),x=e.endTime?new Date(e.endTime).toLocaleString("id-ID",{dateStyle:"short",timeStyle:"short"}):new Date().toLocaleString("id-ID",{dateStyle:"short",timeStyle:"short"}),y=e.duration||Ue(e.startTime,e.endTime||Date.now()),k=(parseFloat(e.startingCash)||0)+(parseFloat(e.cashSales)||0),l=e.actualCash!==void 0?parseFloat(e.actualCash):k,w=l-k,M=w===0?"SEIMBANG (PAS)":w>0?`LEBIH (+${m(w)})`:`KURANG (-${m(Math.abs(w))})`,O=window.open("","_blank",`width=${s?460:360},height=740`);if(!O){document.getElementById("pos-shift-receipt-modal")?.remove(),document.body.insertAdjacentHTML("beforeend",`
        <div id="pos-shift-receipt-modal" class="fixed inset-0 z-[10003] flex items-center justify-center p-3 sm:p-4" style="background:rgba(15,23,42,0.7);backdrop-filter:blur(4px)">
            <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full ${s?"max-w-md":"max-w-sm"} border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div id="pos-shift-receipt-paper-box" class="p-4 overflow-y-auto flex-1 font-mono text-[11px] bg-slate-50/60 dark:bg-slate-950 text-slate-800 dark:text-slate-200 space-y-1.5 select-text">
                    ...
                    <div class="flex justify-between"><span>Transfer Bank</span><span>${m(e.bankSales||0)}</span></div>
                    ...
                    <div class="flex justify-between"><span>Penjualan Tunai</span><span>${m(e.cashSales||0)}</span></div>
                    <div class="flex justify-between font-bold"><span>Kas Sistem</span><span>${m(k)}</span></div>
                    ${t?"":`
                    <div class="flex justify-between font-bold"><span>Kas Fisik Dihitung</span><span>${m(l)}</span></div>
                    <div class="border-t border-dashed border-slate-300 dark:border-slate-700 my-1"></div>
        ...
        </div>`);return}O.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Slip Rekap Shift Kasir</title>
    ...
    <tr><td>Tunai (Cash)</td><td style="text-align:right">${m(e.cashSales||0)}</td></tr>
    <tr><td>QRIS</td><td style="text-align:right">${m(e.qrisSales||0)}</td></tr>
    <tr><td>Transfer Bank</td><td style="text-align:right">${m(e.bankSales||0)}</td></tr>
    <tr><td>Tempo (Piutang)</td><td style="text-align:right">${m(e.tempoSales||0)}</td></tr>
    <tr><td colspan="2"><div class="line"></div></td></tr>
    <tr class="total"><td>TOTAL OMSET</td><td style="text-align:right">${m(e.totalSales||0)}</td></tr>
    ${(e.discountTotal||0)>0?`<tr><td>Diskon Toko</td><td style="text-align:right">-${m(e.discountTotal)}</td></tr>`:""}
    </table>
    <div class="line"></div>
    <p class="left" style="font-weight:bold">REKONSILIASI KAS LACI:</p>
    <table>
    <tr><td>Modal Awal</td><td style="text-align:right">${m(e.startingCash)}</td></tr>
    <tr><td>Penjualan Tunai</td><td style="text-align:right">${m(e.cashSales||0)}</td></tr>
    <tr style="font-weight:bold"><td>Kas Sistem</td><td style="text-align:right">${m(k)}</td></tr>
    ${t?"":`
    <tr style="font-weight:bold"><td>Kas Fisik Laci</td><td style="text-align:right">${m(l)}</td></tr>
    <tr><td colspan="2"><div class="line"></div></td></tr>
    ...
    </body></html>`),O.document.close()},We=()=>{const e=typeof Y=="function"?Y():{deviceType:"system"},t=i("pos-shift-receipt-paper-box");if(t)if(e.deviceType==="rawbt"&&window.AndroidNativeApp&&typeof window.AndroidNativeApp.printRawBT=="function"){const a=t.innerText,s=btoa(unescape(encodeURIComponent(a)));window.AndroidNativeApp.printRawBT(s)}else window.AndroidNativeApp&&typeof window.AndroidNativeApp.print=="function"?window.AndroidNativeApp.print():window.print()},fe=()=>{const e=[i("pos-shift-btn-storefront"),i("pos-shift-btn-admin")],t=B();e.forEach(a=>{a&&(t&&t.status==="open"?a.id==="pos-shift-btn-storefront"?a.innerHTML=`
    ...
    <div class="space-y-4">
    ...
    </div>`,await Bt())},Bt=async()=>{const e=i("admin-shift-list-target");if(e)try{const t=await $.collection("freshmart").doc("cms_data").collection("pos_shifts").orderBy("startTime","desc").limit(50).get();if(t.empty){e.innerHTML=`
            ...
            <div class="p-4 sm:p-5 rounded-2xl sm:rounded-3xl bg-white dark:bg-slate-800/95 border border-slate-200/90 dark:border-slate-700/80 shadow-2xs hover:shadow-xs transition-all space-y-3">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                    ...
                    <div class="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800/70">
                        ...
                        <span class="font-bold text-slate-800 dark:text-slate-200 block mt-0.5">${m(r.startingCash)}</span>
                    </div>
                    <div class="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800/70">
                        <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 block">Total Omset</span>
                        <span class="font-black text-emerald-600 dark:text-emerald-400 block mt-0.5">${m(r.totalSales||0)}</span>
                    </div>
                    <div class="p-2.5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-100 dark:border-slate-800/70">
                        <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 block">Kas Fisik Laci</span>
                        <span class="font-black text-slate-900 dark:text-white block mt-0.5">${m(r.actualCash!==void 0?r.actualCash:(r.startingCash||0)+(r.cashSales||0))}</span>
                    </div>
                </div>
        ...
        <div class="text-center py-10 text-rose-500 text-xs">
        ...
        </div>`}};window.getActiveShift=B;window.saveActiveShift=Ve;window.clearActiveShift=jt;window.getLastClosedShift=ma;window.isShiftActive=pe;window.openPOSOpenShiftModal=oe;window.closePOSOpenShiftModal=Ae;window.posSetStartCashPreset=ha;window.posUpdateStartCashChips=Ht;window.confirmStartPOSShift=wa;window.recordTransactionToShift=Dt;window.openPOSShiftModal=se;window.openPOSShiftSummaryModal=se;window.closePOSShiftSummaryModal=ze;window.openPOSCloseShiftModal=Ge;window.closePOSCloseShiftModal=Oe;window.setPOSCountMode=ka;window.calcPOSDenominations=Nt;window.updatePOSShiftDiscrepancy=Rt;window.confirmClosePOSShift=va;window.printShiftSettlementReceipt=Qe;window.executeShiftPrintDirect=We;window.renderShiftHeaderBadge=fe;window.renderAdminShiftReportView=Sa;window.loadAdminShiftReports=Bt;let kt=!1;const Et=()=>kt?Promise.resolve():pa(()=>import("./pos-variant-sheet-DWhoZtix.js"),__vite__mapDeps([0,1])).then(()=>{kt=!0});let g=[],z="",te="",U="grid";try{const e=localStorage.getItem("pos_view_mode");(e==="list"||e==="grid")&&(U=e)}catch{}let f={name:"",phone:"",isMember:!1,memberId:null,isNewTempo:!1},j="cash",R=0,E=0,H="rp",A=0,V="",vt=null,de=null,ae=null,Se=null,ce=null,Pe=!0,Ee="environment",me=!1,W=null,yt="",St=0;const Je=e=>{U=e;try{localStorage.setItem("pos_view_mode",e)}catch{}document.querySelectorAll("#pos-view-btn-grid").forEach(t=>{e==="grid"?(t.style.background="var(--color-primary)",t.className="w-8 h-8 rounded-lg flex items-center justify-center text-xs transition-all cursor-pointer text-white shadow-xs"):(t.style.removeProperty("background"),t.className="w-8 h-8 rounded-lg flex items-center justify-center text-xs transition-all cursor-pointer text-slate-500 hover:text-slate-800 dark:text-slate-400")}),document.querySelectorAll("#pos-view-btn-list").forEach(t=>{e==="list"?(t.style.background="var(--color-primary)",t.className="w-8 h-8 rounded-lg flex items-center justify-center text-xs transition-all cursor-pointer text-white shadow-xs"):(t.style.removeProperty("background"),t.className="w-8 h-8 rounded-lg flex items-center justify-center text-xs transition-all cursor-pointer text-slate-500 hover:text-slate-800 dark:text-slate-400")}),N()},J=e=>Math.max(0,parseInt(e)||0),h=e=>ua(e),Z=()=>g.reduce((e,t)=>e+t.subtotal,0),F=()=>{if(H==="percent"){const e=Math.min(100,Math.max(0,parseFloat(A)||0));return Math.round(Z()*e/100)}return Math.min(Z(),J(A||E))},I=()=>Math.max(0,Z()-F()),Ft=()=>R-I(),be=e=>{if(!e)return{isManaged:!1,totalStock:0,isOutOfStock:!0,isLowStock:!1};if(!(u?.store?.useStock!==!1))return{isManaged:!1,totalStock:9999,isOutOfStock:!1,isLowStock:!1};let a=0;return Array.isArray(e.variants)&&e.variants.length>0?a=e.variants.reduce((s,r)=>s+(r&&r.stock!=null&&parseFloat(r.stock)||0),0):a=parseFloat(e.stock)||0,{isManaged:!0,totalStock:a,isOutOfStock:a<=0,isLowStock:a>0&&a<=5}},K=()=>{try{const e=window.AudioContext||window.webkitAudioContext;if(!e)return;const t=new e,a=t.createOscillator(),s=t.createGain();a.type="sine",a.frequency.setValueAtTime(1400,t.currentTime),s.gain.setValueAtTime(.08,t.currentTime),s.gain.exponentialRampToValueAtTime(1e-4,t.currentTime+.08),a.connect(s),s.connect(t.destination),a.start(),a.stop(t.currentTime+.08),setTimeout(()=>{t.close().catch(()=>{})},150)}catch{}},Pa=(e,t)=>{if(!e||!e.wholesale||!e.wholesale.length)return null;const a=[...e.wholesale].sort((s,r)=>r.minQty-s.minQty);for(const s of a)if(t>=parseFloat(s.minQty))return parseFloat(s.price);return null},G=e=>{if(!e.isVariant){const t=(u.products||[]).find(s=>s&&String(s.id)===String(e.id)),a=t?Pa(t,e.qty):null;a!==null?(e.basePrice=e.basePrice||e.price,e.price=a,e.isWholesale=!0):(e.basePrice&&(e.price=e.basePrice),e.isWholesale=!1)}return e.subtotal=Math.max(0,e.price*e.qty-J(e.discount)),e},Ta=()=>{const e=new Date,t=a=>String(a).padStart(2,"0");return`POS-${e.getFullYear()}${t(e.getMonth()+1)}${t(e.getDate())}-${Date.now().toString(36).toUpperCase()}`},Kt=()=>{de&&clearInterval(de);const e=()=>{const a=new Date().toLocaleTimeString("id-ID",{hour:"2-digit",minute:"2-digit",second:"2-digit"})+" WIB";document.querySelectorAll("#pos-live-clock").forEach(s=>{s.textContent=a})};e(),de=setInterval(e,1e3)},_t=()=>{de&&(clearInterval(de),de=null)};window.stopPOSClock=_t;const Le=()=>{window.__posBarcodeFn&&(document.removeEventListener("keydown",window.__posBarcodeFn),window.__posBarcodeFn=null)},qt=()=>{Le(),window.__posBarcodeFn=e=>{if(!e||typeof e.key!="string")return;const t=window.curViewName||"";if(!(t==="view-pos-cashier"||t==="view-admin"&&window.cTab==="pos"))return;if(e.key==="F4"){e.preventDefault();const r=i("pos-search-input");r&&(r.focus(),r.select());return}if(e.key==="F6"||e.key==="F7"){e.preventDefault(),He();return}if(e.key==="F8"){e.preventDefault(),he();return}if(e.key==="F9"){e.preventDefault(),i("pos-camera-scanner-modal")?re():Ne();return}if(e.key==="F10"){e.preventDefault(),pe()?se():oe();return}const s=document.activeElement?.tagName?.toLowerCase();if(!(s==="input"||s==="textarea"||s==="select"))if(e.key==="Enter"){if(V&&V.length>=3){const r=V.trim().toLowerCase(),n=(u.products||[]).find(o=>o&&o.isActive!=="false"&&o.isActive!==!1&&(o.barcode&&o.barcode.toLowerCase()===r||o.sku&&o.sku.toLowerCase()===r||o.id&&String(o.id).toLowerCase()===r));if(n)je(n.id),K(),v(`Ditambahkan: ${n.name}`,"success");else{const o=i("pos-search-input");o&&(o.value=V,z=V,N()),v("Barcode tidak ditemukan di katalog","warning")}V=""}}else e.key&&e.key.length===1&&(V=(V||"")+e.key,clearTimeout(vt),vt=setTimeout(()=>{V=""},150))},document.addEventListener("keydown",window.__posBarcodeFn)},je=e=>{const t=(u.products||[]).find(n=>n&&String(n.id)===String(e));if(!t)return;if(t.variants&&t.variants.length>0){Et().then(()=>{typeof window.openPOSVariantSheet=="function"&&window.openPOSVariantSheet(e)});return}const s=be(t);s.isManaged&&s.isOutOfStock&&v(`Peringatan: Stok "${t.name}" habis di etalase/gudang!`,"warning");const r=g.find(n=>String(n.id)===String(e)&&!n.isVariant);if(r){if(s.isManaged&&r.qty+1>s.totalStock){v(`Stok maksimal "${t.name}" hanya ${s.totalStock} ${t.unit||"pcs"}`,"warning");return}r.qty+=1,G(r)}else{const n=parseFloat(t.price)||0;g.push(G({id:t.id,name:t.name,price:n,basePrice:n,qty:1,discount:0,subtotal:n,isVariant:!1,isWholesale:!1}))}K(),typeof window.triggerHaptic=="function"&&window.triggerHaptic("light"),C()},Vt=(e,t)=>{const a=(u.products||[]).find(n=>n&&String(n.id)===String(e));if(!a)return;const s=be(a),r=g.find(n=>String(n.id)===String(e)&&!n.isVariant);if(r){if(s.isManaged&&r.qty+t>s.totalStock){v(`Stok maksimal "${a.name}" hanya ${s.totalStock} ${a.unit||"pcs"}`,"warning");return}r.qty+=t,G(r)}else{const n=parseFloat(a.price)||0,o=G({id:a.id,name:a.name,price:n,basePrice:n,qty:t,discount:0,subtotal:n*t,isVariant:!1,isWholesale:!1});g.push(o)}K(),typeof window.triggerHaptic=="function"&&window.triggerHaptic("light"),C()},Ut=(e,t,a,s,r=1)=>{const n=`${e}__v${s}`,o=g.find(c=>c.cartKey===n);if(o)o.qty+=r,G(o);else{const p=`${(u.products||[]).find(b=>b&&String(b.id)===String(e))?.name||e} — ${t}`;g.push(G({id:e,cartKey:n,name:p,variantName:t,variantIdx:s,price:a,basePrice:a,qty:r,discount:0,subtotal:a*r,isVariant:!0,isWholesale:!1}))}K(),typeof window.triggerHaptic=="function"&&window.triggerHaptic("light"),C()},zt=(e,t)=>{const a=g.find(s=>(s.cartKey||String(s.id))===String(e));if(a){if(t>0&&!a.isVariant){const s=(u.products||[]).find(r=>r&&String(r.id)===String(a.id));if(s){const r=be(s);if(r.isManaged&&a.qty+t>r.totalStock){v(`Stok maksimal tersedia: ${r.totalStock} ${s.unit||"pcs"}`,"warning");return}}}a.qty=Math.max(1,a.qty+t),G(a),t>0&&K(),C()}},Gt=(e,t)=>{const a=g.find(r=>(r.cartKey||String(r.id))===String(e));if(!a)return;let s=Math.max(1,J(t));if(!a.isVariant){const r=(u.products||[]).find(n=>n&&String(n.id)===String(a.id));if(r){const n=be(r);n.isManaged&&s>n.totalStock&&(v(`Stok maksimal tersedia: ${n.totalStock} ${r.unit||"pcs"}`,"warning"),s=n.totalStock)}}a.qty=s,G(a),C()},Qt=(e,t)=>{const a=g.find(s=>(s.cartKey||String(s.id))===String(e));a&&(a.discount=Math.min(J(t),a.price*a.qty),G(a),C())},Wt=e=>{g=g.filter(t=>(t.cartKey||String(t.id))!==String(e)),C(),typeof window.triggerHaptic=="function"&&window.triggerHaptic("light")},Jt=()=>{if(g.length===0)return;const e=()=>{g=[],E=0,A=0,H="rp",C(),v("Keranjang kasir dikosongkan.")};typeof window.showConfirm=="function"?window.showConfirm("Kosongkan Keranjang","Hapus semua item dari transaksi saat ini?",e,"Ya, Kosongkan",!0):e()},ge=(e="hold")=>{try{const t=window.AudioContext||window.webkitAudioContext;if(!t)return;const a=new t,s=a.createOscillator(),r=a.createGain();s.type="sine";const n=a.currentTime;e==="hold"?(s.frequency.setValueAtTime(659.25,n),s.frequency.exponentialRampToValueAtTime(880,n+.1)):(s.frequency.setValueAtTime(880,n),s.frequency.exponentialRampToValueAtTime(1174.66,n+.1)),r.gain.setValueAtTime(.08,n),r.gain.exponentialRampToValueAtTime(1e-4,n+.16),s.connect(r),r.connect(a.destination),s.start(),s.stop(n+.16),setTimeout(()=>{a.close().catch(()=>{})},200)}catch{}},$a=e=>{if(!e)return"";const t=Math.floor((Date.now()-e)/1e3);if(t<45)return"Baru saja";const a=Math.floor(t/60);if(a<60)return`${a} mnt lalu`;const s=Math.floor(a/60);return s<24?`${s} jam lalu`:new Date(e).toLocaleDateString("id-ID",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"})};let P=[];try{const e=localStorage.getItem("pos_held_carts");if(e){const t=JSON.parse(e);Array.isArray(t)&&(P=t)}}catch{P=[]}const Ie=()=>{try{localStorage.setItem("pos_held_carts",JSON.stringify(P))}catch{}ne()},ne=()=>{const e=P.length,t=i("pos-held-btn-storefront"),a=i("pos-held-btn-admin");t&&(e>0?t.innerHTML=`
            ...
            </button>`)},He=()=>{if(g.length===0){v("Keranjang masih kosong, tidak ada transaksi untuk ditahan.","warning");return}const e=f?.name?`Antrean #${P.length+1} — ${f.name}`:`Antrean #${P.length+1}`,t=g.reduce((s,r)=>s+(r.qty||1),0),a=I();ue(!0),typeof window.pushModalHistory=="function"&&window.pushModalHistory("posHoldPrompt"),document.getElementById("pos-hold-prompt-modal")?.remove(),document.body.insertAdjacentHTML("beforeend",`
    <div id="pos-hold-prompt-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4" style="background:rgba(15,23,42,0.65);backdrop-filter:blur(4px)">
    ...
    </div>`),setTimeout(()=>{const s=i("pos-hold-note-input");s&&(s.focus(),s.select())},50)},De=(e=!1)=>{const t=i("pos-hold-prompt-modal");t&&(!e&&typeof window.requestCloseModal=="function"?window.requestCloseModal("posHoldPrompt",!1,()=>t.remove()):t.remove())},Ye=()=>{if(g.length===0)return;const t=(i("pos-hold-note-input")?.value||"").trim()||`Antrean #${P.length+1}`,a={id:`HELD-${Date.now().toString(36).toUpperCase()}`,time:Date.now(),note:t,cart:JSON.parse(JSON.stringify(g)),globalDisc:F(),discountType:H,discountVal:A,customer:{...f},total:I(),subtotal:Z(),itemCount:g.reduce((s,r)=>s+(r.qty||1),0)};P.unshift(a),Ie(),g=[],E=0,A=0,H="rp",f={name:"",phone:"",isMember:!1,memberId:null,isNewTempo:!1},De(),C(),N(),ge("hold"),v(`Antrean "${t}" berhasil diparkir!`,"success")},he=(e=!1)=>{!e&&typeof window.pushModalHistory=="function"&&window.pushModalHistory("posHeldModal"),document.getElementById("pos-held-list-modal")?.remove();const t=P.length,a=t===0?`
        ...
        <div class="divide-y divide-slate-100 dark:divide-slate-800">
            ${P.map((s,r)=>{const n=d(s.id),o=(s.cart||[]).slice(0,3).map(p=>`${d(p.name)} (${p.qty}x)`).join(", "),c=(s.cart||[]).length>3?` +${s.cart.length-3} lainnya`:"";return`
    ...
    <div id="pos-held-list-modal" class="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4" style="background:rgba(15,23,42,0.65);backdrop-filter:blur(4px)">
    ...
    </div>`)},we=(e=!1)=>{const t=i("pos-held-list-modal");t&&(!e&&typeof window.requestCloseModal=="function"?window.requestCloseModal("posHeldModal",!1,()=>t.remove()):t.remove())},Ze=e=>{const t=P.findIndex(a=>a.id===e);if(t===-1){v("Transaksi tertahan tidak ditemukan.","warning");return}if(g.length>0){document.getElementById("pos-recall-confirm-modal")?.remove(),document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);return}Xe(t)},Xe=e=>{const t=P[e];t&&(g=JSON.parse(JSON.stringify(t.cart||[])),H=t.discountType||"rp",A=t.discountVal!==void 0?t.discountVal:t.globalDisc||0,E=F(),f=t.customer?{...t.customer}:{name:"",phone:"",isMember:!1,memberId:null,isNewTempo:!1},P.splice(e,1),Ie(),we(),C(),N(),ge("recall"),v(`Antrean "${t.note}" berhasil dipanggil kembali!`,"success"))},et=e=>{document.getElementById("pos-recall-confirm-modal")?.remove();const t=f?.name?`Antrean #${P.length+1} — ${f.name}`:`Antrean #${P.length+1}`,a={id:`HELD-${Date.now().toString(36).toUpperCase()}`,time:Date.now(),note:t,cart:JSON.parse(JSON.stringify(g)),globalDisc:F(),discountType:H,discountVal:A,customer:{...f},total:I(),subtotal:Z(),itemCount:g.reduce((r,n)=>r+(n.qty||1),0)};P.unshift(a);const s=P.findIndex(r=>r.id===e);s!==-1?Xe(s):(Ie(),we())},tt=e=>{document.getElementById("pos-recall-confirm-modal")?.remove();const t=P.findIndex(a=>a.id===e);t!==-1&&Xe(t)},at=e=>{const t=P.find(a=>a.id===e);t&&(document.getElementById("pos-delete-confirm-modal")?.remove(),document.body.insertAdjacentHTML("beforeend",`
    <div id="pos-delete-confirm-modal" class="fixed inset-0 z-[10005] flex items-center justify-center p-4" style="background:rgba(15,23,42,0.75);backdrop-filter:blur(4px)">
        <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-xs border border-slate-200 dark:border-slate-800 p-6 text-center transform transition-all">
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400 mb-5 leading-relaxed">
                Antrean <span class="font-bold text-slate-800 dark:text-slate-200">"${d(t.note)}"</span> (${t.itemCount} item • ${h(t.total)}) akan dihapus permanen.
            </p>
            <div class="flex gap-2.5">
               ...
               </div>`:t.map(o=>{if(!o)return"";const c=!!(o.img&&typeof o.img=="string"&&o.img.trim()),p=c?Be(o.img,"w300-rw"):"",b=Array.isArray(o.variants)&&o.variants.length>0,x=Array.isArray(o.wholesale)&&o.wholesale.length>0,k=g.filter(S=>S&&String(S.id)===String(o.id)).reduce((S,D)=>S+(D&&D.qty?D.qty:0),0),l=d(String(o.id!=null?o.id:"")),w=be(o),M=d(String(o.name||"Produk")),O=d(String(o.category||"")),ye=parseFloat(o.price)||0;return U==="list"?`
                    <div class="pos-list-item${k>0?" in-cart":""}${w.isOutOfStock?" opacity-75":""}" onclick="window.posAddToCart('${l}')">
                        <div class="pos-list-thumb">
                            ${c?`<img width="52" height="52" loading="lazy" decoding="async" src="${d(p)}" alt="${M}" onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex';">
                        ...
                        <div style="flex:1;min-width:0">
                            <div style="display:flex;align-items:center;gap:4px;flex-wrap:wrap;margin-bottom:3px">
                                ${O?`<span style="font-size:9px;text-transform:uppercase;letter-spacing:0.06em;font-weight:700;color:#94a3b8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:80px">${O}</span>`:""}
                                ${b?'<span class="pos-badge pos-badge-varian"><i class="fa-solid fa-layer-group" style="font-size:6px"></i> VARIAN</span>':""}
                                ${x?'<span class="pos-badge pos-badge-grosir"><i class="fa-solid fa-tags" style="font-size:6px"></i> GROSIR</span>':""}
                                ${w.isOutOfStock?'<span class="pos-badge pos-badge-habis"><i class="fa-solid fa-ban" style="font-size:6px"></i> HABIS</span>':""}
                                ${w.isLowStock?`<span class="pos-badge pos-badge-low"><i class="fa-solid fa-triangle-exclamation" style="font-size:6px"></i> SISA ${w.totalStock}</span>`:""}
                            </div>
                            <p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate" title="${M}">${M}</p>
                            <p style="font-size:12px;font-weight:900;color:var(--color-primary);margin-top:2px">${h(ye)}</p>
                        </div>
                        <button onclick="event.stopPropagation();window.posAddToCart('${l}')" class="pos-add-btn" title="Tambah ke keranjang">
                            <i class="fa-solid fa-plus"></i>
                        </button>
                    </div>`:`
                <div class="pos-product-card${k>0?" in-cart":""}${w.isOutOfStock?" opacity-75":""}" onclick="window.posAddToCart('${l}')">
                    ...
                    <div class="pos-img-box">
                        ...
                        ${c?`<img width="300" height="300" loading="lazy" decoding="async" src="${d(p)}" alt="${M}"
                    ...
                    <div class="pos-card-info">
                        ${O?`<p class="pos-card-cat">${O}</p>`:""}
                        <p class="pos-card-name" title="${M}">${M}</p>
                        <div class="pos-card-footer">
                            <span class="pos-card-price">${h(ye)}</span>
                            <button onclick="event.stopPropagation();window.posAddToCart('${l}')" class="pos-add-btn" title="Tambah ke keranjang">
                                <i class="fa-solid fa-plus"></i>
                            </button>
                        </div>
                ...
                </div>`})}},C=()=>{const e=g.reduce((l,w)=>l+w.qty,0),t=Z(),a=I(),s=h(a),r=h(t),n=g.length===0?`<div class="flex flex-col items-center justify-center h-full py-12 text-slate-300 dark:text-slate-600 select-none">
            <div class="w-16 h-16 rounded-3xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 mb-3 shadow-inner">
           ...
           </div>`:g.map(l=>{const w=d(String(l.cartKey||l.id)),M=Ca(l),O=l.isVariant&&l.variantName?d(l.name.replace(` — ${l.variantName}`,"")):d(l.name);return`
            <div class="group flex items-center gap-2.5 p-2 sm:p-2.5 bg-white dark:bg-slate-800/90 rounded-2xl border border-slate-200/90 dark:border-slate-700/80 shadow-xs hover:border-[var(--color-primary)] transition-all">
                ...
                <div class="w-10 h-10 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-700 shrink-0 border border-slate-100 dark:border-slate-700 flex items-center justify-center">
                    ${M?`<img width="40" height="40" loading="lazy" src="${d(M)}" alt="${d(l.name)}" onerror="this.onerror=null; this.style.display='none'; this.nextElementSibling.style.display='flex';" class="w-full h-full object-cover">
                ...
                <div class="flex-1 min-w-0">
                    <p class="text-xs font-bold text-slate-800 dark:text-slate-100 truncate leading-snug" title="${d(l.name)}">${O}</p>
                    <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        ...
                        <span class="text-[10px] text-slate-500 font-medium">
                            ${l.isWholesale&&l.basePrice?`<span class="line-through text-slate-400">${h(l.basePrice)}</span> <span class="font-bold" style="color:var(--color-primary)">${h(l.price)}</span>`:h(l.price)}
                        </span>
                    </div>
                    <div class="flex items-center gap-1 mt-1">
                        <span class="text-[9px] text-slate-400 font-bold uppercase">Diskon:</span>
                        <input type="number" min="0" placeholder="0" value="${l.discount||""}" onchange="window.posSetItemDisc('${w}',this.value)"
                ...
                <div class="flex flex-col items-end gap-1 shrink-0">
                    <div class="flex items-center gap-0.5 bg-slate-100 dark:bg-slate-700/80 rounded-lg p-0.5 border border-slate-200 dark:border-slate-600 focus-within:border-[var(--color-primary)] transition-colors">
                        ...
                        <input type="number" min="1" value="${l.qty}" onchange="window.posSetQty('${w}',this.value)"
                            class="w-6 text-center text-[11px] font-black bg-transparent text-slate-800 dark:text-slate-100 focus:outline-none">
                        <button onclick="window.posUpdateQty('${w}',1)" class="w-5 h-5 rounded text-slate-600 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-600 font-black text-xs flex items-center justify-center cursor-pointer active:scale-90 transition-all">+</button>
                    </div>
                    <p class="text-xs font-black" style="color:var(--color-primary)">${h(l.subtotal)}</p>
                    <button onclick="window.posRemoveItem('${w}')" class="p-1 rounded-md text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 text-[11px] transition-all cursor-pointer" title="Hapus item">
                        <i class="fa-solid fa-trash-can"></i>
                    </button>
                </div>
            </div>`}).join("");document.querySelectorAll(".pos-cart-items-target").forEach(l=>l.innerHTML=n),document.querySelectorAll(".pos-subtotal-target").forEach(l=>l.textContent=r),document.querySelectorAll(".pos-total-target").forEach(l=>l.textContent=s),document.querySelectorAll(".pos-item-count-target").forEach(l=>l.textContent=String(e));const o=F(),c=h(o);document.querySelectorAll(".pos-disc-val-input").forEach(l=>{document.activeElement!==l&&(l.value=A||"")}),document.querySelectorAll(".pos-global-disc-target").forEach(l=>{document.activeElement!==l&&(l.value=A||"")}),document.querySelectorAll(".pos-disc-preview-target").forEach(l=>{l.textContent=o>0?`- ${c}`:"Rp 0",o>0?(l.classList.remove("text-slate-400"),l.classList.add("text-rose-500")):(l.classList.add("text-slate-400"),l.classList.remove("text-rose-500"))}),document.querySelectorAll(".pos-disc-type-rp").forEach(l=>{H==="rp"?(l.className="pos-disc-type-rp px-2.5 py-0.5 rounded-md transition-all cursor-pointer font-black text-white shadow-xs text-[10px]",l.style.background="var(--color-primary)",l.style.color="#ffffff"):(l.className="pos-disc-type-rp px-2.5 py-0.5 rounded-md transition-all cursor-pointer text-slate-600 dark:text-slate-300 hover:text-slate-900 font-bold text-[10px]",l.style.background="transparent",l.style.color="")}),document.querySelectorAll(".pos-disc-type-pct").forEach(l=>{H==="percent"?(l.className="pos-disc-type-pct px-2.5 py-0.5 rounded-md transition-all cursor-pointer font-black text-white shadow-xs text-[10px]",l.style.background="var(--color-primary)",l.style.color="#ffffff"):(l.className="pos-disc-type-pct px-2.5 py-0.5 rounded-md transition-all cursor-pointer text-slate-600 dark:text-slate-300 hover:text-slate-900 font-bold text-[10px]",l.style.background="transparent",l.style.color="")}),document.querySelectorAll(".pos-disc-prefix").forEach(l=>{l.textContent=H==="percent"?"%":"Rp",l.style.color="var(--color-primary)"});const p=[5,10,15,20,50],b=[2e3,5e3,1e4,25e3,5e4],x=(l,w)=>H===w&&Number(A)===Number(l),y=H==="percent"?`
        ${p.map(l=>{const w=x(l,"percent");return`<button onclick="window.posApplyQuickDiscount(${l},'percent')" 
                class="px-2 py-0.5 rounded-md text-[9px] font-black cursor-pointer transition-all ${w?"shadow-xs text-white":"hover:brightness-95"}"
                style="${w?"background:var(--color-primary);color:#ffffff;border:1px solid var(--color-primary);":"background:rgba(var(--color-primary-rgb),0.1);color:var(--color-primary);border:1px solid rgba(var(--color-primary-rgb),0.25);"}">${l}%</button>`}).join("")}
        ${A>0?`<button onclick="window.posApplyQuickDiscount(0,'percent')" class="px-2 py-0.5 rounded-md bg-rose-500/10 hover:bg-rose-500/20 text-[9px] font-black text-rose-600 dark:text-rose-400 border border-rose-500/25 cursor-pointer transition-all active:scale-95">Reset</button>`:""}
        `:`
        ${b.map(l=>{const w=x(l,"rp"),M=`${l/1e3}rb`;return`<button onclick="window.posApplyQuickDiscount(${l},'rp')" 
                class="px-2 py-0.5 rounded-md text-[9px] font-black cursor-pointer transition-all ${w?"shadow-xs text-white":"hover:brightness-95"}"
                style="${w?"background:var(--color-primary);color:#ffffff;border:1px solid var(--color-primary);":"background:rgba(var(--color-primary-rgb),0.1);color:var(--color-primary);border:1px solid rgba(var(--color-primary-rgb),0.25);"}">${M}</button>`}).join("")}
        ${A>0?`<button onclick="window.posApplyQuickDiscount(0,'rp')" class="px-2 py-0.5 rounded-md bg-rose-500/10 hover:bg-rose-500/20 text-[9px] font-black text-rose-600 dark:text-rose-400 border border-rose-500/25 cursor-pointer transition-all active:scale-95">Reset</button>`:""}
        `;document.querySelectorAll(".pos-disc-chips-target").forEach(l=>l.innerHTML=y),document.querySelectorAll(".pos-pay-btn-target").forEach(l=>{l.disabled=g.length===0;const w=l.querySelector(".btn-text");w&&(w.textContent=g.length>0?`BAYAR — ${s}`:"PROSES PEMBAYARAN")}),document.querySelectorAll(".pos-hold-btn-target").forEach(l=>{l.disabled=g.length===0,g.length===0?l.classList.add("opacity-40","cursor-not-allowed"):l.classList.remove("opacity-40","cursor-not-allowed")}),ne();const k=i("pos-mobile-floating-bar");k&&(g.length>0?(k.classList.remove("translate-y-32","opacity-0","pointer-events-none"),k.classList.add("translate-y-0","opacity-100")):(k.classList.add("translate-y-32","opacity-0","pointer-events-none"),k.classList.remove("translate-y-0","opacity-100"),ue(!0)))},Yt=()=>{if(g.length===0){v("Keranjang masih kosong!","warning");return}typeof window.pushModalHistory=="function"&&window.pushModalHistory("posPayment"),f={name:"",phone:"",isMember:!1,memberId:null,isNewTempo:!1},j="cash",R=I(),X(),ke(),document.body.insertAdjacentHTML("beforeend",`
    <div id="pos-pay-modal" class="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4" style="background:rgba(15,23,42,0.65);backdrop-filter:blur(4px)">
      <div class="bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md max-h-[94vh] flex flex-col overflow-hidden border border-slate-200/80 dark:border-slate-800">
        ...
        <div class="p-4 sm:p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center shrink-0 bg-slate-50/60 dark:bg-slate-800/40">
          <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Total Tagihan: <span class="font-black text-sm" style="color:var(--color-primary)">${h(I())}</span></p>
          </div>
      ...
      <div class="flex justify-between items-center py-2 px-3 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-100 dark:border-slate-700/60 mb-2.5 text-xs">
        ...
        `).join("");t.innerHTML=`
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <input id="pos-paid-input" type="number" min="0" placeholder="${a}" value="${R||""}"
                ...
                <!-- Kembalian Box -->
                <div id="pos-change-box" class="mt-2.5 p-3 rounded-2xl border transition-all flex items-center justify-between ${R>=a?"bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800":"bg-rose-50 dark:bg-rose-950/30 border-rose-200 dark:border-rose-800"}">
                    <div>
                        <p class="text-[9px] font-black uppercase tracking-wider text-slate-400">Status Kembalian</p>
                        <p id="pos-change-label" class="text-xs font-bold ${R>=a?"text-emerald-700 dark:text-emerald-400":"text-rose-700 dark:text-rose-400"}">
                            ${R>=a?"Kembalian Uang Pembeli:":"Uang Masih Kurang:"}
                        </p>
                    </div>
                    <span id="pos-change-display" class="text-base font-black ${R>=a?"text-emerald-700 dark:text-emerald-400":"text-rose-600 dark:text-rose-400"}">
                        ${h(Math.abs(Ft()))}
                    </span>
                </div>
            </div>
        `}else if(e==="qris"){const r=u.payment?.qrisUrl||"";t.innerHTML=`
          ...
          </div>`))},nt=e=>{j=e,Zt("pos-pay",e,["cash","qris","transfer","tempo"]),Fe(e),e==="transfer"&&(!u.banks||!u.banks.length)&&ke().then(t=>{j==="transfer"&&t&&t.length>0&&Fe("transfer")})},it=e=>{R=J(e);const t=I(),a=R-t,s=i("pos-change-display"),r=i("pos-change-label"),n=i("pos-change-box"),o=i("pos-process-btn");s&&(s.textContent=h(Math.abs(a))),r&&(r.textContent=a>=0?"Kembalian Uang Pembeli:":"Uang Masih Kurang:"),s&&(s.className=`text-base font-black ${a>=0?"text-emerald-700 dark:text-emerald-400":"text-rose-600 dark:text-rose-400"}`),n&&(n.className=`mt-2.5 p-3 rounded-2xl border transition-all flex items-center justify-between ${a>=0?"bg-emerald-50 dark:bg-emerald-950/30 border-emerald-200 dark:border-emerald-800":"bg-rose-50 dark:bg-rose-950/30 border-rose-200 dark:border-rose-800"}`),o&&j==="cash"&&(o.disabled=a<0,o.classList.toggle("opacity-50",a<0))},lt=e=>{const t=i("pos-paid-input");t&&(t.value=e,it(e),typeof window.triggerHaptic=="function"&&window.triggerHaptic("light"))},ke=async()=>{if(Array.isArray(u.banks)&&u.banks.length>0)return u.banks;try{const e=await $.collection("freshmart").doc("cms_data").get();if(e.exists){const t=e.data();if(Array.isArray(t?.banks)&&t.banks.length>0)return u.banks=t.banks,u.banks}}catch{}return u.banks||[]},X=async()=>{if(u.customers&&u.customers.length>0)return u.customers;try{const e=await $.collection("freshmart").doc("cms_data").collection("customers").get();return u.customers=e.docs.map(t=>({...t.data(),id:t.id,_docId:t.id})),u.customers}catch{return u.customers||[]}},ea=(e,t)=>{if(!e||!t||!t.length)return[];const a=e.trim().toLowerCase(),s=a.replace(/\D/g,"");let r=s;r.startsWith("62")?r=r.slice(2):r.startsWith("0")&&(r=r.slice(1));const n=[],o=new Set;return t.forEach(c=>{if(!c)return;const p=String(c.id||c._docId||c.phone||"");if(o.has(p))return;const b=String(c.phone||"").replace(/\D/g,"");let x=b;x.startsWith("62")?x=x.slice(2):x.startsWith("0")&&(x=x.slice(1));const y=String(c.name||"").toLowerCase();let k=!1;r.length>=4&&x&&(x===r||x.endsWith(r)||r.endsWith(x)||b.includes(s))&&(k=!0),!k&&(p.toLowerCase()===a||p===s)&&(k=!0),!k&&a.length>=2&&y.includes(a)&&(k=!0),k&&(o.add(p),n.push(c))}),n},Ma=async e=>{if(!e)return null;const t=e.trim(),a=t.replace(/\D/g,"");let s=a;s.startsWith("62")?s=s.slice(2):s.startsWith("0")&&(s=s.slice(1));const r=$.collection("freshmart").doc("cms_data").collection("customers"),o=Array.from(new Set([s?"62"+s:null,s?"0"+s:null,s||null,s?"+62"+s:null,a||null,t].filter(Boolean))).map(async b=>{try{const x=await r.doc(b).get();if(x&&x.exists)return{...x.data(),id:x.id,_docId:x.id}}catch{}return null}),p=(await Promise.all(o)).find(Boolean);if(p){u.customers||(u.customers=[]);const b=u.customers.findIndex(x=>String(x.id||x.phone)===String(p.id||p.phone));return b>-1?u.customers[b]=p:u.customers.push(p),p}try{const b=await r.limit(300).get();if(!b.empty){u.customers=b.docs.map(y=>({...y.data(),id:y.id,_docId:y.id}));const x=ea(e,u.customers);if(x.length>0)return x[0]}}catch{}return null},$e=e=>{f.isMember=!0,f.name=e.name||"Member Toko",f.phone=e.phone||"",f.memberId=e.id||e._docId||e.phone,f.points=parseFloat(e.points)||0;const t=i("pos-cust-phone");t&&(t.value=e.phone||e.name||"");const a=f.points,s=typeof window.getMemberTier=="function"?window.getMemberTier(a):{badge:"MEMBER RESMI"},r=i("pos-member-result");r&&(r.innerHTML=`
          ...
          <button onclick="window.resetPosMember()" type="button" class="shrink-0 px-2.5 py-1.5 rounded-xl text-[10px] font-bold text-slate-600 hover:text-rose-600 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 transition-all cursor-pointer" title="Ganti Member">
        ...
        </div>`),v(`Member terdeteksi: ${e.name} (${a} Poin)`,"success")},dt=e=>{const a=(u.customers||[]).find(s=>s&&String(s.id||s._docId||s.phone)===String(e));a&&$e(a)},ct=()=>{f.isMember=!1,f.name="",f.phone="",f.memberId=null,f.points=0;const e=i("pos-cust-phone");e&&(e.value="",e.focus());const t=i("pos-member-result");t&&(t.innerHTML="")};let Pt=null;const pt=()=>{clearTimeout(Pt);const e=i("pos-cust-phone")?.value?.trim()||"";if(!e){if(!f.memberId){const s=i("pos-member-result");s&&(s.innerHTML="")}return}const t=e.replace(/\D/g,"");!(Array.isArray(u.customers)&&u.customers.length>0)&&t.length<10&&e.length<8||(Pt=setTimeout(()=>{ve()},350))},ve=async()=>{const t=i("pos-cust-phone")?.value?.trim()||"";if(!t){v("Masukkan nomor HP atau nama member","warning");return}const a=i("pos-member-result"),s=i("pos-member-lookup-btn");s&&(s.disabled=!0,s.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>'),a&&(a.innerHTML='<div class="p-2.5 text-center text-xs text-slate-400"><i class="fa-solid fa-spinner fa-spin mr-1.5"></i>Memeriksa database member...</div>');try{await X();const r=ea(t,u.customers||[]);if(r.length===1)$e(r[0]);else if(r.length>1)a.innerHTML=`
            ...
            `;else{const n=await Ma(t);if(n)$e(n);else{f.isMember=!1,f.name="",f.memberId=null,f.points=0;const c=t.replace(/\D/g,"").length>=8;a.innerHTML=`
                  <div class="p-3 bg-amber-50 dark:bg-amber-950/30 rounded-xl border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300 text-xs space-y-1">
                    ...
                    `}
                  </div>`}}}catch(r){console.error("[POS] Error lookupPosMember:",r),a&&(a.innerHTML=`<p class="text-xs text-rose-500 p-2">Gagal memeriksa data: ${d(r.message||"Koneksi error")}</p>`)}finally{s&&(s.disabled=!1,s.innerHTML='<i class="fa-solid fa-magnifying-glass mr-1.5"></i><span>Cek</span>')}},ta=async()=>{if(g.length===0){v("Keranjang kosong!","warning");return}const e=f.isMember?f.name||"Member Toko":i("pos-cust-name")?.value?.trim()||"Pelanggan Umum",t=f.isMember?f.phone||i("pos-cust-phone")?.value?.trim()||"":i("pos-cust-phone")?.value?.trim()||"";if(f.isNewTempo&&!t){v("No. HP wajib diisi untuk tempo!","warning");return}if(j==="cash"&&(R=J(i("pos-paid-input")?.value||0),R<I())){v(`Uang kurang! Minimal ${h(I())}`,"warning");return}f.name=e,f.phone=t;const a=j==="tempo"?J(i("pos-dp-input")?.value||0):0,s=j==="transfer"&&i("pos-bank-sel")?.value||"",r=i("pos-process-btn");r&&(r.disabled=!0,r.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i>Memproses...');const n=u.store?.useStock===!0||u.store?.useStock==="true";if(n)for(const o of g){const c=(u.products||[]).find(b=>String(b.id)===String(o.id));if(!c)continue;const p=parseFloat(o.qty)||0;if(o.variantName&&c.variants){const b=(c.variants||[]).find(y=>y.name===o.variantName),x=parseFloat(b&&b.stock!==void 0?b.stock:0);if(x<p){v(`Stok ${o.name} (${o.variantName}) tidak cukup! Sisa: ${x}`,"warning"),r&&(r.disabled=!1,r.innerHTML='<i class="fa-solid fa-check-circle mr-2"></i>Selesaikan Transaksi');return}}else{const b=parseFloat(c.stock!==void 0?c.stock:0);if(b<p){v(`Stok ${o.name} tidak cukup! Sisa: ${b}`,"warning"),r&&(r.disabled=!1,r.innerHTML='<i class="fa-solid fa-check-circle mr-2"></i>Selesaikan Transaksi');return}}}try{const o=Ta(),c=typeof window.getCashierSession=="function"?window.getCashierSession():null,p=c?.name||u.store?.name||"Kasir",b=c?.uid||window.__currentAdminUid||"admin",x=new Date().toISOString(),y=Re.firestore.FieldValue.serverTimestamp(),k=j==="tempo"?"Diproses":"Selesai",l=B(),w=l&&l.status==="open"?l.id:null,M=l&&l.status==="open"?l.shiftNo||l.id:null,O={orderId:o,txId:o,source:"pos",channel:"pos",status:k,timestamp:y,dateString:x,dateMs:Date.now(),shiftId:w,shiftNo:M,cashier:b,cashierName:p,customer:{name:e,phone:t,wa:t,address:"Beli Langsung di Kasir (POS)",deliveryMethod:"takeaway",isMember:!!f.isMember,memberId:f.memberId||null},customerName:e,customerPhone:t,customerType:f.isMember?"Member":"Pelanggan Umum",items:g.map(S=>({id:S.id,name:S.name,price:parseFloat(S.price)||0,basePrice:parseFloat(S.basePrice||S.price)||0,qty:parseFloat(S.qty)||1,discount:parseFloat(S.discount)||0,subtotal:parseFloat(S.subtotal)||0,variantName:S.variantName||"",isVariant:!!S.isVariant,isWholesale:!!S.isWholesale,effectivePrice:parseFloat(S.price)||0})),payment:{method:j,subtotal:Z(),productDiscount:J(E),shippingCost:0,grandTotal:I(),paid:j==="cash"?R:j==="tempo"?a:I(),change:j==="cash"?Ft():0,bank:s,paymentStatus:j==="tempo"?"hutang":"lunas",tempoDp:a,tempoBalance:j==="tempo"?I()-a:0,tempoDueDate:Date.now()+30*24*60*60*1e3,tempoPenaltyRate:1,tempoPenaltyStopped:!1},subtotal:Z(),globalDiscount:F(),discountType:H,discountVal:A,total:I(),isTempo:j==="tempo",pointsEarned:0,notes:""};if(f.isMember&&t){const D=(typeof window.calculateCartPoints=="function"?window.calculateCartPoints(g,u.store):{totalPoints:0}).totalPoints||0;if(D>0){O.pointsEarned=D;try{const _=t.replace(/\D/g,""),T=String(f.memberId||_);if(await $.collection("freshmart").doc("cms_data").collection("customers").doc(T).set({points:Re.firestore.FieldValue.increment(D),lastOrderAt:x},{merge:!0}),u.customers){const q=u.customers.find(L=>L&&(String(L.id)===T||String(L.phone).replace(/\D/g,"")===_));q&&(q.points=(parseFloat(q.points)||0)+D)}}catch(_){console.warn("[POS] Gagal update poin member:",_)}}}if(await $.collection("freshmart_orders").doc(o).set(O),Dt(O),n){const S=[];for(const D of g){const _=String(D.id),T=(u.products||[]).find(L=>String(L.id)===_);if(!T)continue;const xe=parseFloat(D.qty)||0,q={};if(D.variantName&&T.variants){const L=T.variants.findIndex(da=>da.name===D.variantName);L>-1&&(T.variants[L].stock=Math.max(0,(parseFloat(T.variants[L].stock)||0)-xe),T.variants[L].stock===0&&(T.variants[L].isActive=!1),T.variants[L].totalSold=(parseFloat(T.variants[L].totalSold)||0)+xe,q.variants=T.variants)}else T.stock=Math.max(0,(parseFloat(T.stock)||0)-xe),q.stock=T.stock,T.stock===0&&(T.isActive="false",q.isActive="false"),T.totalSold=(parseFloat(T.totalSold)||0)+xe,q.totalSold=T.totalSold;try{await $.collection("freshmart").doc("cms_data").collection("products").doc(_).update(q),S.push(_)}catch(L){console.warn("[POS] Gagal update stok produk di Firestore:",_,L)}}if(S.length>0)try{await $.collection("freshmart").doc("cms_data").update({lastUpdate:Re.firestore.FieldValue.increment(1),updateType:"stock_change",updatedProductIds:S})}catch{}}ot(),ue(!0);const ye={...O};g=[],E=0,A=0,H="rp",C(),N(),Aa(ye)}catch(o){console.error("[POS] Error:",o),v("Gagal menyimpan transaksi. Coba lagi.","error"),r&&(r.disabled=!1,r.innerHTML='<i class="fa-solid fa-check-circle mr-2"></i>Selesaikan Transaksi')}},Aa=e=>{const t=e.payment.method==="cash"?`<p class="text-sm text-slate-500">Kembalian: <span class="font-black text-emerald-600">${h(e.payment.change)}</span></p>`:e.payment.method==="tempo"?'<p class="text-sm text-amber-600 font-semibold">⚠️ Dicatat sebagai Piutang Tempo</p>':`<p class="text-sm text-slate-500">Metode: ${e.payment.method.toUpperCase()}</p>`,a=JSON.stringify(e).replace(/"/g,"&quot;"),s=!!document.getElementById("pos-admin-container")||typeof window.cTab=="function"&&window.cTab()==="pos";document.body.insertAdjacentHTML("beforeend",`
    <div id="pos-success-modal" class="fixed inset-0 z-[9999] flex items-center justify-center p-4" style="background:rgba(15,23,42,0.65);backdrop-filter:blur(4px)">
      <div class="bg-white dark:bg-slate-900 rounded-3xl shadow-2xl w-full max-w-sm border border-slate-200/80 dark:border-slate-800">
    ...
    </div>`)},aa=e=>{document.getElementById("pos-success-modal")?.remove();const t=typeof Y=="function"?Y():{paperSize:"58mm"},a=t.paperSize==="80mm",s=t.headerText||u.store?.name||"TOKO PUTRI",r=u.store?.wa||"",n=u.store?.address||"",o=t.footerText||"Terima Kasih Atas Kunjungan Anda!",c=new Date(e.dateMs||Date.now()).toLocaleString("id-ID"),p=(e.items||[]).map(y=>`<tr><td style="padding:2px 0;word-wrap:break-word">${d(y.name)}</td><td style="text-align:right;padding:2px 4px;white-space:nowrap">${y.qty}x ${h(y.price)}</td><td style="text-align:right;padding:2px 0;white-space:nowrap;font-weight:bold">${h(y.subtotal)}</td></tr>`).join(""),b=e.discountType==="percent"&&e.discountVal?`Diskon (${e.discountVal}%)`:"Diskon",x=window.open("","_blank",`width=${a?460:360},height=720`);if(!x){document.getElementById("pos-receipt-fallback-modal")?.remove(),document.body.insertAdjacentHTML("beforeend",`
        ...
        </div>`);return}x.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Struk POS</title>
    ...
    <script>window.onload=()=>{window.print();setTimeout(()=>window.close(),800)}<\/script>
    </body></html>`),x.document.close()},bt=()=>{const e=typeof Y=="function"?Y():{deviceType:"system"},t=i("pos-receipt-paper-box");if(t)if(e.deviceType==="rawbt"&&window.AndroidNativeApp&&typeof window.AndroidNativeApp.printRawBT=="function"){const a=t.innerText,s=btoa(unescape(encodeURIComponent(a)));window.AndroidNativeApp.printRawBT(s)}else window.AndroidNativeApp&&typeof window.AndroidNativeApp.print=="function"?window.AndroidNativeApp.print():window.print()},sa=({isStorefront:e})=>{const a=(typeof window.getCashierSession=="function"?window.getCashierSession():null)?.name||(e?"Kasir":"Admin Seller"),s=d(u.store?.name||"Toko Putri");return`
    ...
    `},ra=()=>{try{z="",te="",g=[],E=0;const e=i("view-pos-cashier");if(!e)return;const t=i("admin-content"),a=i("view-admin");t&&a?.classList.contains("admin-pos-mode")&&(t.innerHTML="",a.classList.remove("admin-pos-mode")),e.innerHTML=sa({isStorefront:!0}),N(),C(),ne(),fe(),qt(),Kt(),X(),na(),setTimeout(()=>{pe()||oe()},350)}catch(e){console.error("Gagal render POS Storefront:",e)}},oa=()=>{try{z="",te="";const e=i("view-admin");e&&e.classList.add("admin-pos-mode");const t=i("view-pos-cashier");if(t&&(t.innerHTML=""),!i("admin-content"))return;ba("admin-content",`
        ...
        `),N(),C(),ne(),fe(),qt(),Kt(),X(),na(),setTimeout(()=>{pe()||oe()},350)}catch(e){console.error("Gagal render POS Admin:",e);const t=i("admin-content");t&&(t.innerHTML=`

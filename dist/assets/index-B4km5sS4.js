...
Kami akan segera memproses pesanan Anda. Terima kasih! 🙏`;window.open(`https://wa.me/${r}?text=${encodeURIComponent(p)}`,"_blank")}catch{D(),b("Gagal memuat data pesanan!")}},Lo=e=>{Ce("Hapus Pesanan","Yakin ingin hapus permanen?",async()=>{if(!Ge){te(!0),U("Menghapus...");try{await $.collection("freshmart_orders").doc(e).delete(),b("Terhapus!"),it===e&&Bs()}catch{b("Gagal!")}finally{te(!1),D()}}})};window.exportOrdersToExcel=So;window.rAdmOrd=To;window.openOrderDetail=ha;window.saveOrderCustomerToDB=Ao;window.ackRewardClaim=Mo;window.closeOrderDetailModal=Bs;window.updateOrderStatus=Do;window.konfirmasiKeWA=Co;window.deleteOrder=Lo;const Io=()=>{const e=n.store.name||"Toko Grosir",t=n.store.themeColor||"#10b981",a=(o,i,l=!1)=>{let d=document.querySelector(`meta[${l?"property":"name"}="${o}"]`);d||(d=document.createElement("meta"),l?d.setAttribute("property",o):d.setAttribute("name",o),document.head.appendChild(d)),d.setAttribute("content",i)};a("theme-color",t),a("mobile-web-app-capable","yes"),a("apple-mobile-web-app-capable","yes"),a("apple-mobile-web-app-status-bar-style","black-translucent"),a("apple-mobile-web-app-title",e),a("application-name",e),a("msapplication-TileColor",t),document.title=e,localStorage.setItem("freshmart_theme_color",t),n.store.uiTheme&&n.store.uiTheme!==localStorage.getItem("freshmart_ui_theme")&&(localStorage.setItem("freshmart_ui_theme",n.store.uiTheme),ht(n.store.uiTheme));const r=n.store.bgStyle||localStorage.getItem("freshmart_bg_style")||"minimalist",s=n.store.bgCustomUrl!==void 0?n.store.bgCustomUrl:localStorage.getItem("freshmart_bg_custom_url")||"";vt(r,s)},Os=()=>{A("admin-content",`
    ...
    `)},$a=()=>{const e=xt(),t=oe===0?`Tahun ${re}`:`${Oe[oe-1]} ${re}`,a=e.omset-e.disc-e.hpp,r=oe===0?null:`${re}-${oe}`,s=ya(),o=a-s,i=n.taxSettings?.taxScheme||"umkm_final";let l,d,p;i==="umkm_final"?(l=.5,d=e.omset,p="PPh Final UMKM (0,5% × Omset)"):i==="badan_normal"?(l=22,d=Math.max(0,o),p="PPh Badan (22% × Laba Bersih)"):(l=parseFloat(n.taxSettings?.customTaxRate)||0,d=Math.max(0,o),p=`PPh Custom (${l}% × Laba Bersih)`);const f=d*(l/100),g=o-f;let u="";if(oe===0)u=Array.from({length:12},(w,v)=>v+1).map(w=>{const v=`${re}-${w}`,h=(n.taxSettings?.monthlyExpenses||{})[v]||0;return`<div class="flex items-center justify-between gap-2 py-2 border-b border-slate-100 dark:border-slate-700/50 last:border-0">
    ...
    `),rAdmItms(e)};window.rAdmItms=e=>{e&&(Ye=e,typeof window.setCTab=="function"&&window.setCTab(e),window.cTab=e);const t=m("admin-list-container"),a=t?t.closest(".scroll-content"):null,r=a?a.scrollTop:0;if(e==="products"&&m("admin-product-stats")){const i=Ot();A("admin-product-stats",`
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <div class="card-modern p-5 sm:p-5">

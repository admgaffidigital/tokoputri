import{d as H,_ as R}from"./module-member-CGh5W0Mi.js";import{a as h,e as c,t as g,i,b as S,r as K,f as G,x as ce,u as pe}from"./module-print-DoqR8CYY.js";import{f as F}from"./vendor-firebase-core-D2OF5R23.js";import"./vendor-firebase-db-BIUZcnOd.js";let p=[],O="",P="",x={name:"",phone:"",isMember:!1,memberId:null,isNewTempo:!1},u="cash",T=0,M=0,$="",U=null;const k=e=>Math.max(0,parseInt(e)||0),d=e=>G(e),q=()=>p.reduce((e,t)=>e+t.subtotal,0),f=()=>Math.max(0,q()-k(M)),Y=()=>Math.max(0,T-f()),j=e=>(e.subtotal=Math.max(0,e.price*e.qty-k(e.discount)),e),be=()=>{const e=new Date,t=a=>String(a).padStart(2,"0");return`POS-${e.getFullYear()}${t(e.getMonth()+1)}${t(e.getDate())}-${Date.now().toString(36).toUpperCase()}`},me=()=>{window.__posBarcodeFn&&document.removeEventListener("keydown",window.__posBarcodeFn),window.__posBarcodeFn=e=>{const t=document.activeElement?.tagName?.toLowerCase();if(!(t==="input"||t==="textarea"||t==="select"))if(e.key==="Enter"){if($.length>=3){const a=$.trim().toLowerCase(),o=(h.products||[]).find(s=>s&&s.isActive!=="false"&&s.isActive!==!1&&(s.barcode&&s.barcode.toLowerCase()===a||s.sku&&s.sku.toLowerCase()===a||s.id&&String(s.id).toLowerCase()===a));if(o)A(o.id),g(`Ditambahkan: ${o.name}`,"success");else{const s=c("pos-search-input-d");s&&(s.value=$,O=$,C()),g("Barcode tidak ditemukan di katalog","warning")}$=""}}else e.key.length===1&&($+=e.key,clearTimeout(U),U=setTimeout(()=>{$=""},150))},document.addEventListener("keydown",window.__posBarcodeFn)},A=e=>{const t=(h.products||[]).find(o=>o&&String(o.id)===String(e));if(!t)return;const a=p.find(o=>String(o.id)===String(e));if(a)a.qty+=1,j(a);else{const o=parseFloat(t.price)||0;p.push(j({id:t.id,name:t.name,price:o,qty:1,discount:0,subtotal:o}))}y()},J=(e,t)=>{const a=p.find(o=>String(o.id)===String(e));a&&(a.qty=Math.max(1,a.qty+t),j(a),y())},W=(e,t)=>{const a=p.find(o=>String(o.id)===String(e));a&&(a.qty=Math.max(1,k(t)),j(a),y())},X=(e,t)=>{const a=p.find(o=>String(o.id)===String(e));a&&(a.discount=Math.min(k(t),a.price*a.qty),j(a),y())},Z=e=>{p=p.filter(t=>String(t.id)!==String(e)),y()},ee=()=>{p=[],M=0,y()},C=()=>{const e=(h.products||[]).filter(l=>{if(!l||l.isActive==="false"||l.isActive===!1||P&&l.category!==P)return!1;if(O){const b=O.toLowerCase();return(l.name||"").toLowerCase().includes(b)||(l.barcode||"").toLowerCase().includes(b)||(l.sku||"").toLowerCase().includes(b)}return!0}),a=["Semua",...new Set((h.products||[]).filter(l=>l&&l.isActive!=="false"&&l.category).map(l=>l.category))].map(l=>{const b=l==="Semua",r=b?!P:P===l;return`<button onclick="window.posCatFilter('${i(b?"":l)}')" class="shrink-0 px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border transition-all ${r?"text-white border-transparent":"bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border-slate-200 dark:border-slate-700"}" style="${r?"background:var(--color-primary)":""}">${i(l)}</button>`}).join(""),o=e.length===0?'<div class="col-span-full flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-600"><i class="fa-solid fa-box-open text-4xl mb-3"></i><p class="font-semibold text-sm">Produk tidak ditemukan</p></div>':e.map(l=>{const b=l.img?ce(l.img,"w200-rw"):"",r=p.find(v=>String(v.id)===String(l.id));return`<button onclick="window.posAddToCart('${i(String(l.id))}')" class="relative flex flex-col bg-white dark:bg-slate-800 border rounded-2xl p-2.5 text-left transition-all duration-150 hover:-translate-y-0.5 hover:shadow-md active:scale-95 overflow-hidden ${r?"border-[var(--color-primary)] shadow-sm":"border-slate-200 dark:border-slate-700"}">
    ...
    <div id="pos-pay-modal" class="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center p-0 sm:p-4" style="background:rgba(0,0,0,0.5)">
      <div class="bg-white dark:bg-slate-900 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-md max-h-[92vh] flex flex-col">
        ...
        <input id="pos-dp-input" type="number" min="0" placeholder="0" value="0" class="w-full border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm text-right bg-white dark:bg-slate-800 focus:outline-none focus:border-[var(--color-primary)]">`)},oe=e=>{x.isMember=e==="member",x.isNewTempo=e==="tempo",ae("pos-ctype",e,["umum","member","tempo"]);const t=c("pos-customer-fields");t&&(e==="umum"?t.innerHTML='<input id="pos-cust-name" type="text" placeholder="Nama pelanggan (opsional)" class="w-full border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm bg-white dark:bg-slate-800 focus:outline-none focus:border-[var(--color-primary)]">':e==="member"?t.innerHTML=`<div class="flex gap-2"><input id="pos-cust-phone" type="tel" placeholder="No. HP Member" class="flex-1 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2.5 text-sm bg-white dark:bg-slate-800 focus:outline-none focus:border-[var(--color-primary)]">
    ...
    </div>`)},ne=e=>{document.getElementById("pos-success-modal")?.remove();const t=h.store?.name||"TOKO PUTRI",a=h.store?.wa||"",o=h.store?.address||"",s=new Date(e.dateMs).toLocaleString("id-ID"),n=(e.items||[]).map(b=>`<tr><td style="padding:2px 0;word-wrap:break-word">${i(b.name)}</td><td style="text-align:right;padding:2px 4px;white-space:nowrap">${b.qty}x ${d(b.price)}</td><td style="text-align:right;padding:2px 0;white-space:nowrap">${d(b.subtotal)}</td></tr>`).join(""),l=window.open("","_blank","width=420,height=720");if(!l){g("Izinkan popup untuk cetak struk","warning");return}l.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Struk POS</title>
    ...
    </div>`),C(),y(),me(),window.posAddToCart=A,window.posUpdateQty=J,window.posSetQty=W,window.posSetItemDisc=X,window.posRemoveItem=Z,window.posClearCart=ee,window.openPayModal=te,window.closePayModal=B,window.setPosCustomerType=oe,window.setPosPayMethod=N,window.updatePosChange=re,window.lookupPosMember=le,window.processPOSTx=ie,window.printPOSReceipt=ne,window.posSetGlobalDisc=e=>{M=k(e),y()},window.posCatFilter=e=>{P=e,C()},window.posSearchFn=e=>{O=e,C()},window.openPOSHistory=()=>R(()=>Promise.resolve().then(()=>he),void 0).then(e=>e.renderPOSHistory())},z=Object.freeze(Object.defineProperty({__proto__:null,addToCart:A,clearCart:ee,closePayModal:B,lookupPosMember:le,openPayModal:te,printPOSReceipt:ne,processPOSTx:ie,removeFromCart:Z,renderPOS:xe,setItemDisc:X,setPosCustomerType:oe,setPosPayMethod:N,setQty:W,updatePosChange:re,updateQty:J},Symbol.toStringTag,{value:"Module"})),_=e=>G(e),fe=e=>new Date(e).toLocaleString("id-ID",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"});let D=new Date().toISOString().slice(0,10),w=[],L=null;const Q=()=>{c("pos-hist-list")&&S("pos-hist-list",'<div class="flex justify-center py-12"><i class="fa-solid fa-spinner fa-spin text-3xl text-slate-300"></i></div>'),L&&(L(),L=null);const t=new Date(D);t.setHours(0,0,0,0);const a=new Date(D);a.setHours(23,59,59,999),L=H.collection("freshmart").doc("cms_data").collection("pos_transactions").where("dateMs",">=",t.getTime()).where("dateMs","<=",a.getTime()).onSnapshot(o=>{w=o.docs.map(s=>s.data()).sort((s,n)=>(n.dateMs||0)-(s.dateMs||0)),V()},o=>{console.error("[POS History]",o),g("Gagal memuat riwayat kasir","error"),w=[],V()})},V=()=>{const e=w.reduce((r,m)=>r+(m.status!=="void"&&m.total||0),0),t=w.filter(r=>r.status!=="void").length,a=w.filter(r=>r.status==="void").length,o={};w.filter(r=>r.status!=="void").forEach(r=>{const m=r.payment?.method||"other";o[m]=(o[m]||0)+(r.total||0)});const s={cash:"Tunai",qris:"QRIS",transfer:"Transfer",tempo:"Tempo"},n=Object.entries(o).map(([r,m])=>`<div class="flex justify-between text-xs"><span class="text-slate-500">${s[r]||r}</span><span class="font-bold text-slate-700 dark:text-slate-200">${_(m)}</span></div>`).join(""),l=`
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-center">
            ...
            <p class="text-base font-black" style="color:var(--color-primary)">${_(e)}</p>
        </div>
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 text-center">
    ...
    ${n?`<div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-3 mb-4 space-y-1.5">${n}</div>`:""}`,b=w.length===0?`<div class="flex flex-col items-center justify-center py-16 text-slate-400 dark:text-slate-600"><i class="fa-solid fa-receipt text-4xl mb-3"></i><p class="font-semibold text-sm">Belum ada transaksi</p><p class="text-xs mt-1">${D}</p></div>`:w.map(r=>{const m=r.status==="void",v={cash:"emerald",qris:"blue",transfer:"violet",tempo:"amber"}[r.payment?.method]||"slate",I={cash:"Tunai",qris:"QRIS",transfer:"Transfer",tempo:"Tempo"}[r.payment?.method]||r.payment?.method;return`<div class="bg-white dark:bg-slate-800 border ${m?"border-red-200 dark:border-red-800 opacity-60":"border-slate-200 dark:border-slate-700"} rounded-2xl p-3 space-y-2 ${m?"":"hover:shadow-sm"} transition-all">
                <div class="flex items-start justify-between gap-2">
                    ...
                    <div class="text-right shrink-0">
                        <p class="font-black text-sm ${m?"line-through text-slate-400":""}" style="${m?"":"color:var(--color-primary)"}">${_(r.total)}</p>
                        ${r.payment?.method==="cash"?`<p class="text-[10px] text-slate-400">Kembalian ${_(r.payment.change||0)}</p>`:""}
                    </div>
                </div>
            ...
            </div>`}).join("");S("pos-hist-rekap",l),S("pos-hist-list",b)},de=e=>{pe("Void Transaksi",`Void transaksi ${e}?
Transaksi akan ditandai batal dan tidak dihitung dalam laporan.`,async()=>{try{await H.collection("freshmart").doc("cms_data").collection("pos_transactions").doc(e).update({status:"void"}),g("Transaksi berhasil divoid","success")}catch{g("Gagal void transaksi","error")}},"Ya, Void")},ge=()=>{S("admin-content",`
    <div class="max-w-full pb-10 fade-in-scale">
        ...
        <div class="flex items-center gap-3 mb-4">
            <div class="flex items-center gap-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 shadow-xs">
                ...
                <input type="date" id="pos-hist-date" value="${D}"
    ...
    </div>`),window.posHistChangDate=e=>{D=e;const t=c("pos-hist-date");t&&(t.value=e),Q()},window.voidPOSTx=de,window.printPOSReceiptFromHist=e=>{R(()=>Promise.resolve().then(()=>z),void 0).then(t=>t.printPOSReceipt(e))},window.__openPOSMain=()=>R(()=>Promise.resolve().then(()=>z),void 0).then(e=>e.renderPOS()),Q()},he=Object.freeze(Object.defineProperty({__proto__:null,renderPOSHistory:ge,voidPOSTx:de},Symbol.toStringTag,{value:"Module"}));export{A as addToCart,ee as clearCart,B as closePayModal,le as lookupPosMember,te as openPayModal,ne as printPOSReceipt,ie as processPOSTx,Z as removeFromCart,xe as renderPOS,X as setItemDisc,oe as setPosCustomerType,N as setPosPayMethod,W as setQty,re as updatePosChange,J as updateQty};

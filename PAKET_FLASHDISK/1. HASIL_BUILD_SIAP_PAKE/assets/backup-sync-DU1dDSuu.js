import{d as p}from"./module-member-C2DXNqoh.js";import{a2 as S,a as o,a0 as f,t as l,e as d,b as L,u as j,i as T}from"./module-print-C2-MjUR_.js";import"./vendor-firebase-core-D2OF5R23.js";import"./vendor-firebase-db-BIUZcnOd.js";let C=localStorage.getItem("tokoputri_last_sync")||new Date().toISOString(),D=!1,u={products:0,categories:0,orders:0,customers:0,cashiers:0,shifts:0};const w=e=>{if(!e)return"-";try{return new Date(e).toLocaleDateString("id-ID",{day:"numeric",month:"short",year:"numeric",hour:"2-digit",minute:"2-digit",second:"2-digit"})+" WIB"}catch{return e}},I=async()=>{d("admin-content")&&(u.products=(o.products||[]).length,u.categories=(o.categories||[]).length,L("admin-content",`
...
`),s=`laporan_transaksi_tokoputri_${new Date().toISOString().slice(0,10)}.csv`;O(i,s),f(),l(`Berhasil mengekspor ${t.length} data transaksi ke CSV!`,"success")}catch(e){f(),console.error("[CSVExport] Gagal ekspor pesanan:",e),l("Gagal mengekspor data transaksi: "+e.message,"error")}},O=(e,t)=>{if(window.AndroidNativeApp&&typeof window.AndroidNativeApp.saveOrShareFile=="function"){const r=btoa(unescape(encodeURIComponent(e)));window.AndroidNativeApp.saveOrShareFile(r,t,"text/csv")}else{const r=new Blob([e],{type:"text/csv;charset=utf-8;"}),n=URL.createObjectURL(r),i=document.createElement("a");i.href=n,i.download=t,document.body.appendChild(i),i.click(),i.remove(),setTimeout(()=>URL.revokeObjectURL(n),1e3)}},K=e=>{const t=e.target.files?.[0];if(!t)return;const r=new FileReader;r.onload=async n=>{try{const i=n.target.result,s=JSON.parse(i),a=!!(s._meta&&s.appData),b=!!(s.products&&Array.isArray(s.products));if(!a&&!b){l("Format berkas tidak dikenali! Pastikan berkas cadangan resmi Toko Putri (.json).","error");return}const x=((a?s.appData:s).products||[]).length,k=a&&s.orders?s.orders.length:0,h=a&&s.customers?s.customers.length:0,v=a&&s._meta?.createdAt?w(s._meta.createdAt):t.name;V({fileName:t.name,backupDate:v,productsCount:x,ordersCount:k,custCount:h,isFullBackup:a,backupDataPayload:s})}catch(i){console.error("[RestoreEngine] Gagal membaca berkas cadangan:",i),l("Berkas JSON rusak atau tidak dapat dibaca!","error")}finally{e.target.value=""}},r.readAsText(t)},V=({fileName:e,backupDate:t,productsCount:r,ordersCount:n,custCount:i,isFullBackup:s,backupDataPayload:a})=>{const b=d("modal-pre-restore-inspector");b&&b.remove();const g=`
    ...
    `;document.body.insertAdjacentHTML("beforeend",g);const x=d("btn-execute-restore");x&&(x.onclick=()=>{d("modal-pre-restore-inspector")?.remove(),R(a)})},R=async e=>{S("Membuat Safety Snapshot & Memulihkan Database...");try{const t={timestamp:new Date().toISOString(),appData:JSON.parse(JSON.stringify(o))};localStorage.setItem("tokoputri_safety_snapshot",JSON.stringify(t));const r=!!(e._meta&&e.appData),n=r?e.appData:e;if(Object.assign(o,n),typeof window.saveApp=="function"?await window.saveApp():await p.collection("freshmart").doc("cms_data").set(o,{merge:!0}),r&&e.customers&&Array.isArray(e.customers)){const i=p.batch(),s=p.collection("freshmart").doc("cms_data").collection("customers");e.customers.slice(0,100).forEach(a=>{if(a.phone||a._id){const b=String(a.phone||a._id);i.set(s.doc(b),a,{merge:!0})}}),await i.commit().catch(()=>{})}f(),l("Database toko berhasil dipulihkan secara akurat!","success"),setTimeout(()=>{I()},600)}catch(t){f(),console.error("[RestoreEngine] Gagal mengeksekusi restore:",t),l("Gagal memulihkan database toko: "+t.message,"error")}},G=async()=>{const e=localStorage.getItem("tokoputri_safety_snapshot");if(!e){l("Tidak ada riwayat snapshot keselamatan yang tersimpan!","warning");return}let t;try{t=JSON.parse(e)}catch{l("Snapshot keselamatan rusak!","error");return}if(await j("Batalkan & Rollback Data?",`Apakah Anda yakin ingin membatalkan pemulihan dan mengembalikan data toko ke snapshot tanggal ${w(t.timestamp)}?`)){S("Mengembalikan data ke kondisi semula (Rollback)...");try{Object.assign(o,t.appData),typeof window.saveApp=="function"?await window.saveApp():await p.collection("freshmart").doc("cms_data").set(o,{merge:!0}),localStorage.removeItem("tokoputri_safety_snapshot"),f(),l("Data toko telah berhasil dikembalikan ke kondisi semula!","success"),setTimeout(()=>{I()},600)}catch(n){f(),console.error("[RollbackEngine] Gagal rollback:",n),l("Gagal mengembalikan data: "+n.message,"error")}}},H=()=>{try{const e={timestamp:new Date().toISOString(),appData:JSON.parse(JSON.stringify(o))};localStorage.setItem("tokoputri_quick_snapshot",JSON.stringify(e)),l("Snapshot cepat berhasil disimpan di memori perangkat!","success")}catch{l("Memori lokal penuh, gagal menyimpan snapshot!","error")}},J=async()=>{const e=localStorage.getItem("tokoputri_quick_snapshot");if(!e){l("Belum ada snapshot cepat yang disimpan di perangkat ini!","warning");return}let t;try{t=JSON.parse(e)}catch{l("Snapshot kilat rusak!","error");return}await j("Terapkan Snapshot Cepat?",`Pulihkan data toko ke snapshot yang disimpan pada ${w(t.timestamp)}?`)&&R(t.appData)};window.renderBackupSyncView=I;window.triggerRealtimeSync=M;window.downloadFullBackupJSON=$;window.exportProductsCSV=F;window.exportOrdersCSV=U;window.handleRestoreFileSelect=K;window.triggerSafetyRollback=G;window.saveQuickDeviceSnapshot=H;window.restoreQuickDeviceSnapshot=J;export{$ as downloadFullBackupJSON,U as exportOrdersCSV,F as exportProductsCSV,K as handleRestoreFileSelect,I as renderBackupSyncView,J as restoreQuickDeviceSnapshot,H as saveQuickDeviceSnapshot,M as triggerRealtimeSync,G as triggerSafetyRollback};

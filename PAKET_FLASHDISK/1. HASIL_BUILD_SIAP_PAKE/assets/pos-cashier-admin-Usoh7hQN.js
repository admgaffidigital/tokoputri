import{d as c,a as g}from"./module-member-CfTyGNdn.js";import{e as r,u as y,a2 as h,t as u,a0 as m,i as d,b as A}from"./module-print-C2-MjUR_.js";import{r as C}from"./module-pos-CHwTfPWi.js";import{f as x}from"./vendor-firebase-core-D2OF5R23.js";import"./vendor-firebase-db-BIUZcnOd.js";const M=async()=>{r("admin-content")&&(A("admin-content",`
    <div class="space-y-4 p-4 sm:p-6">
        ...
        <div id="cashier-panel-shifts" class="hidden"></div>
    </div>`),await b())},S=a=>{const e=r("tab-btn-cashier-accounts"),t=r("tab-btn-cashier-shifts"),s=r("cashier-panel-accounts"),i=r("cashier-panel-shifts");a==="shifts"?(e&&(e.className="px-4 py-2 rounded-xl text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-all cursor-pointer"),t&&(t.className="px-4 py-2 rounded-xl text-xs font-black bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-xs transition-all cursor-pointer"),s&&s.classList.add("hidden"),i&&(i.classList.remove("hidden"),C(i))):(e&&(e.className="px-4 py-2 rounded-xl text-xs font-black bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-xs transition-all cursor-pointer"),t&&(t.className="px-4 py-2 rounded-xl text-xs font-bold text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-all cursor-pointer"),s&&s.classList.remove("hidden"),i&&i.classList.add("hidden"))},b=async()=>{const a=r("cashier-list-container");if(a)try{const e=await c.collection("freshmart").doc("cms_data").collection("cashier_accounts").orderBy("createdAt","desc").get();if(e.empty){try{localStorage.setItem("pos_has_cashier","false"),await c.collection("freshmart").doc("cms_data").set({hasCashier:!1},{merge:!0})}catch{}typeof window.updatePOSHeaderIcon=="function"&&window.updatePOSHeaderIcon(),a.innerHTML=`
            ...
            </div>`;return}const t=e.docs.some(i=>i.data()?.isActive!==!1);try{localStorage.setItem("pos_has_cashier",t?"true":"false"),await c.collection("freshmart").doc("cms_data").set({hasCashier:t},{merge:!0})}catch{}typeof window.updatePOSHeaderIcon=="function"&&window.updatePOSHeaderIcon();const s=e.docs.map(i=>{const o=i.data(),f=i.id,l=o.isActive!==!1,n=o.createdAt?.toDate?o.createdAt.toDate().toLocaleDateString("id-ID"):"-";return`
    ...
    </div>`),setTimeout(()=>{const a=r("add-cashier-modal-box");a&&a.classList.remove("scale-95");const e=r("new-cashier-name");e&&e.focus()},10)},w=()=>{const a=r("add-cashier-modal");a&&(a.style.opacity="0",setTimeout(()=>a.remove(),200))},L=async()=>{const a=r("new-cashier-name")?.value?.trim()||"",e=r("new-cashier-email")?.value?.trim()||"",t=r("new-cashier-pass")?.value||"",s=r("add-cashier-error"),i=r("save-cashier-btn"),o=l=>{s&&(s.textContent=l,s.classList.remove("hidden"))};if((()=>{s&&s.classList.add("hidden")})(),!a){o("Nama kasir wajib diisi.");return}if(!e||!e.includes("@")){o("Email tidak valid.");return}if(t.length<6){o("Password minimal 6 karakter.");return}i&&(i.disabled=!0,i.innerHTML='<i class="fa-solid fa-spinner fa-spin mr-2"></i>Mendaftarkan...'),h("Membuat akun kasir...");try{const n=(x.apps.find(k=>k.name==="pos-cashier-creator")||x.initializeApp(window.FIREBASE_CONFIG||x.app().options,"pos-cashier-creator")).auth(),p=(await n.createUserWithEmailAndPassword(e,t)).user?.uid;if(!p)throw new Error("UID tidak diterima dari Firebase");await n.signOut(),await c.collection("freshmart").doc("cms_data").collection("cashier_accounts").doc(p).set({uid:p,name:a,email:e,role:"cashier",isActive:!0,createdAt:x.firestore.FieldValue.serverTimestamp(),createdBy:g.currentUser?.uid||"admin"});try{localStorage.setItem("pos_has_cashier","true"),await c.collection("freshmart").doc("cms_data").set({hasCashier:!0},{merge:!0})}catch{}w(),u(`Kasir "${a}" berhasil didaftarkan! ✅`,"success"),await b(),typeof window.updatePOSHeaderIcon=="function"?window.updatePOSHeaderIcon():typeof window.initPOSAuth=="function"&&window.initPOSAuth()}catch(l){console.error("[CashierAdmin] Gagal membuat kasir:",l);const n=l.code||"";o(n==="auth/email-already-in-use"?"Email sudah digunakan oleh akun lain.":n==="auth/invalid-email"?"Format email tidak valid.":n==="auth/weak-password"?"Password terlalu lemah (min. 6 karakter).":"Gagal mendaftarkan: "+(l.message||"Error tidak diketahui")),i&&(i.disabled=!1,i.innerHTML='<i class="fa-solid fa-floppy-disk mr-2"></i>Simpan & Daftarkan')}finally{m()}},E=(a,e,t)=>{document.body.insertAdjacentHTML("beforeend",`

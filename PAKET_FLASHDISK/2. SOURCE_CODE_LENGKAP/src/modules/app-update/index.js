...
export const fetchLatestReleaseInfo = async () => {
    ...
    try {
        ...
        if (res.ok) {
            ...
            cachedReleaseInfo = {
                tagName: data.tag_name || 'v1.8.5',
                name: data.name || 'Toko Putri v1.8.5',
        ...
        const fallbackVer = getLatestVersion(appData) || 'v1.8.5';
        cachedReleaseInfo = {
            tagName: fallbackVer,
...
export const downloadLatestApk = () => {
    ...
    setTimeout(() => {
        ...
        if (text) {
            const ver = cachedReleaseInfo?.tagName || 'v1.8.5';

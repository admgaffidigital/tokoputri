...
Kami akan segera memproses pesanan Anda. Terima kasih! 🙏`;window.open(`https://wa.me/${r}?text=${encodeURIComponent(p)}`,"_blank")}catch{D(),b("Gagal memuat data pesanan!")}},Vo=e=>{Le("Hapus Pesanan","Yakin ingin hapus permanen?",async()=>{if(!Qe){ee(!0),O("Menghapus...");try{await k.collection("freshmart_orders").doc(e).delete(),b("Terhapus!"),ct===e&&Ks()}catch{b("Gagal!")}finally{ee(!1),D()}}})};window.exportOrdersToExcel=Fo;window.rAdmOrd=Bo;window.openOrderDetail=$a;window.saveOrderCustomerToDB=Oo;window.ackRewardClaim=qo;window.closeOrderDetailModal=Ks;window.updateOrderStatus=Uo;window.konfirmasiKeWA=Ho;window.deleteOrder=Vo;const Go=()=>{const e=n.store.name||"Toko Grosir",t=n.store.themeColor||"#10b981",a=(o,i,l=!1)=>{let d=document.querySelector(`meta[${l?"property":"name"}="${o}"]`);d||(d=document.createElement("meta"),l?d.setAttribute("property",o):d.setAttribute("name",o),document.head.appendChild(d)),d.setAttribute("content",i)};a("theme-color",t),a("mobile-web-app-capable","yes"),a("apple-mobile-web-app-capable","yes"),a("apple-mobile-web-app-status-bar-style","black-translucent"),a("apple-mobile-web-app-title",e),a("application-name",e),a("msapplication-TileColor",t),document.title=e,localStorage.setItem("freshmart_theme_color",t),n.store.uiTheme&&n.store.uiTheme!==localStorage.getItem("freshmart_ui_theme")&&(localStorage.setItem("freshmart_ui_theme",n.store.uiTheme),Ve(n.store.uiTheme));const r=n.store.bgStyle||localStorage.getItem("freshmart_bg_style")||"minimalist",s=n.store.bgCustomUrl!==void 0?n.store.bgCustomUrl:localStorage.getItem("freshmart_bg_custom_url")||"";Ge(r,s)},Ws=()=>{P("admin-content",`
    ...
    `)},Aa=()=>{const e=yt(),t=oe===0?`Tahun ${re}`:`${Oe[oe-1]} ${re}`,a=e.omset-e.disc-e.hpp,r=oe===0?null:`${re}-${oe}`,s=Pa(),o=a-s,i=n.taxSettings?.taxScheme||"umkm_final";let l,d,p;i==="umkm_final"?(l=.5,d=e.omset,p="PPh Final UMKM (0,5% × Omset)"):i==="badan_normal"?(l=22,d=Math.max(0,o),p="PPh Badan (22% × Laba Bersih)"):(l=parseFloat(n.taxSettings?.customTaxRate)||0,d=Math.max(0,o),p=`PPh Custom (${l}% × Laba Bersih)`);const f=d*(l/100),g=o-f;let u="";if(oe===0)u=Array.from({length:12},(w,y)=>y+1).map(w=>{const y=`${re}-${w}`,h=(n.taxSettings?.monthlyExpenses||{})[y]||0;return`<div class="flex items-center justify-between gap-2 py-2 border-b border-slate-100 dark:border-slate-700/50 last:border-0">
    ...
    `;e.length===0?a+=`<div class="bg-white dark:bg-slate-800 p-8 text-center rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            </div>`}),a+="</div>"),a+="</div>",P("admin-content",a),setTimeout(()=>{m("total-piutang-header")&&(m("total-piutang-header").innerText=x(t))},100)};const di=e=>{Mr(e),Js()},Js=()=>{const e=Ls||"all",t=(Ot||[]).filter(o=>e==="visible"?o.isVisible!==!1:e==="hidden"?o.isVisible===!1:!0),a=`
    ...
    `},wi=()=>{let e=document.getElementById("guarantee-modal");e||(e=document.createElement("div"),e.id="guarantee-modal",e.className="fixed inset-0 z-[115] bg-slate-900/70 backdrop-blur-sm flex items-end sm:items-center justify-center p-0 sm:p-5",e.onclick=t=>{t.target===e&&Ys()},document.body.appendChild(e)),e.innerHTML=`
    ...
    <div class="relative group/banner-wrapper w-full">
        <div id="banner-slider" class="flex overflow-x-auto gap-4 sm:gap-6 pb-4 pt-2 snap-x hide-scrollbar scroll-smooth" ontouchstart="clearInterval(bannerTmr)" ontouchend="setTimeout(startBannerAutoSlide, 8000)" onmouseenter="clearInterval(bannerTmr)" onmouseleave="startBannerAutoSlide()" onscroll="window.onBannerScroll && window.onBannerScroll()">
            ${n.banners.map((i,l)=>{const d=i.type==="video"&&i.videoUrl,p=!d&&i.link?`onclick="window.open('${c(i.link)}', '_self')"`:"";if(d){const f=Ft(i.videoUrl)||{type:"direct",directUrl:Nt(i.videoUrl),embedUrl:Ds(i.videoUrl)};let g="";return f.type==="youtube"?g=`
                <iframe
...
></i>`,s+="</div>"}),s=`<div class="grid grid-cols-1 lg:grid-cols-2 gap-x-5 gap-y-5 items-start">${s}</div>`,P("admin-modal-form",s),e==="products"&&(rVarsB(),rWholB(),rSpecB());const d=m("admin-modal");d&&d.classList.contains("hidden")&&pushModalHistory("admin"),L("admin-modal"),setTimeout(()=>{m("admin-modal").classList.remove("opacity-0"),m("admin-modal-box").classList.remove("scale-95")},10)};window.rVarsB=()=>{const e=document.getElementById("af-category"),t=e?/\bcat\b/i.test(e.value):!1;let a=`<div class="space-y-5 mb-5">${je.map((r,s)=>{let o=r.isActive!==!1&&r.isActive!=="false";return`
    ...
    `,document.body.appendChild(r);const s=r.querySelector("div");Ia("prompt"),setTimeout(()=>{r.classList.remove("opacity-0"),s.classList.remove("scale-95")},10);const o=r.querySelector("#prompt-input");o.focus(),o.select(),window.closePrompt=(i=!1)=>{!r||!r.parentNode||ja("prompt",i,()=>{r.classList.add("opacity-0"),s.classList.add("scale-95"),setTimeout(()=>r.remove(),300),window.closePrompt=null})},r.querySelector("#prompt-cancel").onclick=()=>window.closePrompt(),r.querySelector("#prompt-ok").onclick=()=>{let i=o.value;window.closePrompt(),a(i)}},Ji=()=>{typeof window.openReceiptPreview=="function"&&window.openReceiptPreview()};window.copyVoucher=Hi;window.showToast=It;window.showToastLoading=Vi;window.hideToast=Gi;window.toggleTheme=Ki;window.showConfirm=Wi;window.closeConfirm=sr;window.executeConfirm=Qi;window.customPrompt=zi;window.checkProPrint=Ji;ar();window.firebase=ue;window.db=k;window.DOMPurify=nr;window.uiPalettes=Ze;window.hexToRgb=Ms;window.applyUITheme=Ve;window.toggleTheme=gr;window.applyBackgroundStyle=Ge;br();const Yi=localStorage.getItem("freshmart_ui_theme")||"emerald";Ve(Yi,localStorage.getItem("freshmart_theme_color"));const Ka=()=>{xr();const e=localStorage.getItem("freshmart_bg_style")||"minimalist",t=localStorage.getItem("freshmart_bg_custom_url")||"";Ge(e,t),Da()};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Ka):Ka();window.onerror=function(e,t,a,r,s){return console.error("Global Error Caught:",e,"at",a,":",r),typeof showToast=="function"&&showToast("Ops, ada kendala sistem."),!1};window.addEventListener("unhandledrejection",function(e){console.warn("Promise Rejection Sentinel:",e.reason)});window.updateSEO=wr;window.injectJSONLD=hr;window.rewardStatusLabel=Bt;window.getYouTubeId=Rt;window.parseVideoUrl=Ft;window.fixDriveVideo=Nt;window.fixDriveVideoPreview=Ds;let Wa=qt;const rr=fe;window.calcTaxDetails=e=>{const t=it.store.ppnEnabled===!0||it.store.ppnEnabled==="true",a=parseFloat(it.store.ppnRate)||11,r=it.store.ppnType||"exclusive";if(!t||e<=0)return{ppnEnabled:!1,ppnRate:0,ppnType:r,ppnAmount:0,dppAmount:Math.max(0,e),grandTotalAdd:0};if(r==="inclusive"){const s=Math.round(e*100/(100+a)),o=e-s;return{ppnEnabled:!0,ppnRate:a,ppnType:"inclusive",ppnAmount:o,dppAmount:s,grandTotalAdd:0}}else{const s=Math.round(e*a/100);return{ppnEnabled:!0,ppnRate:a,ppnType:"exclusive",ppnAmount:s,dppAmount:Math.max(0,e),grandTotalAdd:s}}};typeof requestIdleCallback<"u"?requestIdleCallback(_a,{timeout:5e3}):setTimeout(_a,3e3);let Qa=null,it=JSON.parse(JSON.stringify(rr));window.updateProBadge=()=>{};let jt=[],Et=[],_t=[],za={name:"",address:"",lat:null,lng:null,deliveryMethod:"delivery",distance:0,note:"",wa:""},Ja=null,Ya=null,Xa=null,Za="Semua Produk",es="Semua Merek",ts="",as="newest",ss="grid",rs=1,os=12,is="orders",ls="",ns=null;window.isAdm=!1;window.isPro=!0;let ds=null,cs=0,ps=[],ms=[],us=[],fs=1,bs=[],gs=null,xs=null,ws=null,hs=[],vs=[],ys=null,ks=null,$s,Ss=!1,Ps=null,Ts="all",As="today";try{jt=JSON.parse(we("freshmart_cart"))||[],nt(jt)}catch{}try{Et=JSON.parse(we("freshmart_wishlist"))||[],js(Et)}catch{}try{_t=JSON.parse(we("freshmart_my_orders"))||[],$t(_t)}catch{}history.replaceState({view:"view-catalog"},"","");const Xi=e=>{e&&U("loader-text",e);const t=m("global-loader");t&&(t.style.display="flex")},Zi=()=>{const e=m("global-loader");e&&(e.style.display="none")},el=()=>window.sanitizeCart();window.addEventListener("DOMContentLoaded",async()=>{await loadAppData(),syncAppMeta(),attachRealtimeStockSync(),ge.onAuthStateChanged(async e=>{if(e&&e.uid!==da){await ge.signOut();return}if(e){window.isAdm=!0,window.isPro=!0,localStorage.removeItem("isFreshmartPro"),localStorage.removeItem("freshmart_license_code"),window.updateProBadge&&window.updateProBadge();let t=document.getElementById("view-admin-login");t&&!t.classList.contains("hidden")&&(history.replaceState({view:"view-admin"},"",window.location.href),changeView("view-admin",!0),openAdminMenu(),showToast("Sesi Dipulihkan! Selamat Datang."))}else window.isAdm=!1,window.isPro=!1,window.updateProBadge&&window.updateProBadge(),localStorage.removeItem("isFreshmartPro"),localStorage.removeItem("freshmart_license_code")})});window.el=m;window.show=L;window.hide=$;window.toggleCls=Z;window.setIn=U;window.setH=P;window.setV=se;window.getV=C;window.esc=c;window.fixD=W;window.fCur=x;window.sL=we;window.ssL=te;window.defaultFbC=na;window.fbC=na;window.defApp=rr;window.ADMIN_UID=da;window.sLoad=Xi;window.hLoad=Zi;window.sanitizeCart=el;const H=(e,t,a)=>{try{Object.defineProperty(window,e,{get:t,set:a,configurable:!0})}catch{}};H("GAS_UPLOAD_URL",()=>Wa,e=>{Wa=e});H("confirmCb",()=>Qa,e=>{Qa=e});H("appData",()=>it,e=>{it=e});H("cart",()=>jt,e=>{jt=e,nt(e)});H("wishlist",()=>Et,e=>{Et=e,js(e)});H("myOrders",()=>_t,e=>{_t=e,$t(e)});H("cust",()=>za,e=>{za=e});H("currentMember",()=>Ja,e=>{Ja=e});H("selectedReward",()=>Ya,e=>{Ya=e});H("memberCheckTimer",()=>Xa,e=>{Xa=e});H("aCat",()=>Za,e=>{Za=e});H("aBrand",()=>es,e=>{es=e});H("sQ",()=>ts,e=>{ts=e});H("cSort",()=>as,e=>{as=e});H("cView",()=>ss,e=>{ss=e});H("cPage",()=>rs,e=>{rs=e});H("iPP",()=>os,e=>{os=e});H("cTab",()=>is,e=>{is=e});H("aSq",()=>ls,e=>{ls=e});H("eId",()=>ns,e=>{ns=e});H("cProd",()=>ds,e=>{ds=e});H("cVar",()=>cs,e=>{cs=e});H("tVars",()=>ps,e=>{ps=e});H("tWhol",()=>ms,e=>{ms=e});H("tSpec",()=>us,e=>{us=e});H("cQty",()=>fs,e=>{fs=e});H("oMods",()=>bs,e=>{bs=e});H("aOrdLst",()=>gs,e=>{gs=e});H("aCustLst",()=>xs,e=>{xs=e});H("aRevLst",()=>ws,e=>{ws=e});H("gOrds",()=>hs,e=>{hs=e});H("gReviews",()=>vs,e=>{vs=e});H("cVOrd",()=>ys,e=>{ys=e});H("vouch",()=>ks,e=>{ks=e});H("toastT",()=>$s,e=>{$s=e});H("isSaving",()=>Ss,e=>{Ss=e});H("bannerTmr",()=>Ps,e=>{Ps=e});H("reviewFilterMode",()=>Ts,e=>{Ts=e});H("lastReportPeriod",()=>As,e=>{As=e});

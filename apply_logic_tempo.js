...
const target1 = `window.checkProPrint = () => { openReceiptPreview(); };

window.markTempoPaid = async (orderId) => {`;

const repl1 = `window.checkProPrint = () => { openReceiptPreview(); };
...
const target2 = `        piutangOrders.forEach(o => {
            ...
            if (Date.now() > dueDate) {
                ...
                if (daysLate > 0) {
                    isLate = true;
                    latePenalty = (0.01 * sisa) * daysLate;
                }
            }
            ...
            <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border \${isLate ? 'border-rose-300 dark:border-rose-700 shadow-[0_0_15px_rgba(225,29,72,0.1)]' : 'border-slate-200 dark:border-slate-700 shadow-sm'} relative overflow-hidden group hover:-translate-y-1 transition-all">
                \${isLate ? \`<div class="absolute -right-6 top-4 bg-rose-500 text-white text-[9px] font-black uppercase tracking-widest px-8 py-1 rotate-45 shadow-sm">TERLAMBAT \${daysLate} HARI</div>\` : ''}
                ...
                <div class="space-y-2 mb-4 bg-slate-50 dark:bg-slate-900/50 p-3.5 rounded-xl border border-slate-100 dark:border-slate-700/50">
                    ...
                    <div class="flex justify-between items-center text-xs text-rose-600">
                        <span class="font-bold">Denda (1%/hari)</span>
                        <span class="font-black font-mono">+\${fCur(latePenalty)}</span>
                    </div>\` : ''}
                </div>
                
                <div class="flex justify-between items-center bg-rose-50 text-rose-600 dark:bg-rose-900/20 dark:text-rose-400 p-3.5 rounded-xl border border-rose-100 dark:border-rose-900/30 mb-4">
...
const repl2 = `        piutangOrders.forEach(o => {
            let sisa = o.payment?.tempoBalance || 0;

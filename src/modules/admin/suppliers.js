        ...
        <div class="space-y-5 fade-in max-w-5xl mx-auto pb-24 pt-2">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                ...
                <div class="card-modern p-4 sm:p-5 relative overflow-hidden group">
                    ...
                    <p class="text-xl sm:text-2xl font-black text-indigo-600 dark:text-indigo-400 tracking-tight">${metrics.activePurchasesCount}</p>
            ...
            <div class="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
                <div class="flex gap-2 flex-1 max-w-xl">
                    ...
                    <button 
                        ...
                        class="px-3.5 py-3 rounded-2xl border text-xs font-bold transition-all flex items-center gap-1.5 shrink-0 ${activeSupplierTab === 'has_debt' ? 'primary-bg text-white shadow-glow' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}"
                ...
                <div class="flex items-start gap-3.5 min-w-0">
                    ...
                    <div class="min-w-0 flex-1">
                        <div class="flex items-center gap-2 flex-wrap">
                            ...
                            ${s.code ? `<span class="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-mono font-bold">${esc(s.code)}</span>` : ''}
                            <span class="px-2 py-0.5 rounded-md primary-bg-soft primary-text text-[10px] font-bold">${esc(termLabelMap[s.defaultTerms] || 'Cash')}</span>
                        </div>
                        ...
                        <div class="flex items-center gap-3 text-xs text-slate-500 dark:text-slate-400 mt-1 flex-wrap">
                            ...
                            ${s.bankName && s.bankAccount ? `<span><i class="fa-solid fa-credit-card text-blue-500 mr-1"></i>${esc(s.bankName)}: <b class="font-mono text-slate-700 dark:text-slate-300">${esc(s.bankAccount)}</b></span>` : ''}
                        </div>
...
window.openSupplierFormModal = (supplierId = null) => {
    ...
    setH('modal-supplier-form-content', `
        ...
        <form id="supplier-editor-form" onsubmit="window.saveSupplierForm(event, '${isEdit ? s.id : ''}')" class="p-5 sm:p-6 space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="sf-name" required value="${esc(s.name || '')}" placeholder="Contoh: PT Semen Gresik Abadi" class="admin-input bg-slate-50 dark:bg-slate-900">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="sf-code" value="${esc(s.code || '')}" placeholder="Contoh: SUP-SGA" class="admin-input bg-slate-50 dark:bg-slate-900 font-mono">
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="sf-salesName" value="${esc(s.salesName || '')}" placeholder="Nama kontak sales lapangan" class="admin-input bg-slate-50 dark:bg-slate-900">
                </div>
                ...
                <div>
                    ...
                    <input type="tel" id="sf-phone" required value="${esc(s.phone || '')}" placeholder="Contoh: 081234567890" class="admin-input bg-slate-50 dark:bg-slate-900">
                </div>
                ...
                <div>
                    ...
                    <input type="email" id="sf-email" value="${esc(s.email || '')}" placeholder="sales@perusahaan.com" class="admin-input bg-slate-50 dark:bg-slate-900">
                </div>
            </div>
            ...
            <div class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                    <div>
                        ...
                        <input type="text" id="sf-bankName" value="${esc(s.bankName || '')}" placeholder="BCA / Mandiri / BRI" class="admin-input bg-white dark:bg-slate-800">
                    </div>
                    <div>
                        <label class="block text-[9px] font-bold uppercase text-slate-400 mb-1">Nomor Rekening</label>
                        <input type="text" id="sf-bankAccount" value="${esc(s.bankAccount || '')}" placeholder="1234567890" class="admin-input bg-white dark:bg-slate-800 font-mono">
                    </div>
                    <div>
                        <label class="block text-[9px] font-bold uppercase text-slate-400 mb-1">Atas Nama (A/N)</label>
                        <input type="text" id="sf-bankHolder" value="${esc(s.bankHolder || '')}" placeholder="PT / Nama Pemilik" class="admin-input bg-white dark:bg-slate-800">
                    </div>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="sf-address" rows="2" placeholder="Alamat lengkap supplier..." class="admin-input bg-slate-50 dark:bg-slate-900 resize-none">${esc(s.address || '')}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="sf-notes" rows="2" placeholder="Minimal order 50 sak, jadwal kirim tiap hari Selasa & Kamis..." class="admin-input bg-slate-50 dark:bg-slate-900 resize-none">${esc(s.notes || '')}</textarea>
            </div>
        ...
        <div class="p-5 sm:p-7 border-b border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30">
            ...
            <div class="flex items-center gap-2 mt-6 border-b border-slate-200 dark:border-slate-700">
                <button 
                    ...
                    class="px-4 py-2.5 font-bold text-xs border-b-2 transition-all flex items-center gap-2 ${currentDetailModalTab === 'products' ? 'border-[var(--color-primary)] text-[var(--color-primary)]' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}"
                >
                    <i class="fa-solid fa-boxes-stacked"></i>
                ...
                <button 
                    ...
                    class="px-4 py-2.5 font-bold text-xs border-b-2 transition-all flex items-center gap-2 ${currentDetailModalTab === 'orders' ? 'border-[var(--color-primary)] text-[var(--color-primary)]' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}"
                >
                    <i class="fa-solid fa-cart-flatbed"></i>
                ...
                <button 
                    ...
                    class="px-4 py-2.5 font-bold text-xs border-b-2 transition-all flex items-center gap-2 ${currentDetailModalTab === 'debt' ? 'border-[var(--color-primary)] text-[var(--color-primary)]' : 'border-transparent text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'}"
                >
                    <i class="fa-solid fa-file-invoice-dollar"></i>

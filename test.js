...
const defApp = { 
    store: { 
        ...
        adsEnabled: false   // FITUR BARU: Iklan Google AdSense aktif/nonaktif (default NONAKTIF — admin harus aktifkan manual setelah cek akun AdSense)
    }, 
    ...
    banks: [], banners: [], categories: [], brands: [], products: [], vouchers: [], licenseKey: "",
...
window.isAdm = false; window.isPro = false; // FIX: dulu 'let' lokal (bisa di-bypass via console), sekarang properti window agar dijaga oleh Security Block
...
// --- PATCH: SINKRONISASI LISENSI PERANGKAT BARU ---
    // Jika admin login di perangkat baru, ambil lisensi dari database ke memori lokal
    if (appData.licenseKey && !localStorage.getItem("freshmart_license_code")) {
        localStorage.setItem("freshmart_license_code", appData.licenseKey);
...
window.openCategoryModal = () => {
    ...
    <button onclick="setCat('Semua Produk'); closeCategoryModal()" class="w-full flex items-center gap-3.5 p-3 sm:p-3.5 rounded-2xl border transition-all active:scale-[0.98] ${isActiveAll ? 'bg-emerald-50/80 border-emerald-500 dark:bg-emerald-900/30 dark:border-emerald-500 shadow-[0_0_0_1px_rgba(var(--color-primary-rgb),0.2)]' : 'bg-slate-50 border-slate-200 dark:bg-slate-800/50 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-600'} group">
        <div class="w-11 h-11 sm:w-12 sm:h-12 rounded-xl ${isActiveAll ? 'bg-emerald-500 text-white border-none' : 'bg-white dark:bg-slate-700 text-slate-400 border border-slate-200 dark:border-slate-600 group-hover:text-emerald-500'} flex items-center justify-center shadow-sm shrink-0 overflow-hidden transition-colors">
...
window.openBrandModal = () => {
    ...
    h += `
    <button onclick="setBrand('Semua Merek'); closeBrandModal()" class="flex flex-col items-center justify-start p-2.5 sm:p-3.5 rounded-[1.25rem] border transition-all ${isActiveAll ? 'bg-blue-50/80 border-blue-500 dark:bg-blue-900/30 dark:border-blue-500 shadow-[0_0_0_1px_rgba(59,130,246,0.2)]' : 'bg-slate-50 border-slate-200 dark:bg-slate-800/50 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-600'} group">
        <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-full ${isActiveAll ? 'bg-blue-500 text-white border-none' : 'bg-white dark:bg-slate-700 text-slate-400 border border-slate-200 dark:border-slate-600 group-hover:text-blue-500'} flex items-center justify-center shadow-sm mb-2.5 transition-colors shrink-0">
    ...
    appData.brands.forEach(b => {
        ...
        let imgH = b.img ? `<img loading="lazy" src="${esc(b.img)}" class="w-8 h-8 sm:w-9 sm:h-9 object-contain" ></i>` : `<i class="fa-solid fa-tag text-lg sm:text-xl"></i>`;
        h += `
        <button onclick="setBrand('${esc(b.name)}'); closeBrandModal()" class="flex flex-col items-center justify-start p-2.5 sm:p-3.5 rounded-[1.25rem] border transition-all ${isActive ? 'bg-blue-50/80 border-blue-500 dark:bg-blue-900/30 dark:border-blue-500 shadow-[0_0_0_1px_rgba(59,130,246,0.2)]' : 'bg-slate-50 border-slate-200 dark:bg-slate-800/50 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-600'} group">
            <div class="w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-white dark:bg-slate-800 flex items-center justify-center shadow-sm mb-2.5 text-slate-400 group-hover:text-blue-500 overflow-hidden shrink-0 border border-slate-200 dark:border-slate-600">
                ${imgH}
            </div>
...
window.rDyn = () => {
    ...
    let bHTML = (appData.banners && appData.banners.length) ? `<div id="banner-slider" class="flex overflow-x-auto gap-4 sm:gap-6 pb-6 pt-2 snap-x hide-scrollbar scroll-smooth" ontouchstart="clearInterval(bannerTmr)" ontouchend="setTimeout(startBannerAutoSlide, 3000)" onmouseenter="clearInterval(bannerTmr)" onmouseleave="startBannerAutoSlide()">${appData.banners.map((b,i)=>{
        ...
        return `
        <div ${linkAction} class="w-[88vw] sm:w-[480px] min-h-[180px] sm:min-h-[220px] snap-center shrink-0 rounded-[2rem] relative overflow-hidden group cursor-pointer bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-dark)] shadow-glow hover:-translate-y-1.5 transition-all duration-300 border border-white/20 flex flex-col">
            <div class="absolute inset-0 opacity-20 pointer-events-none" style="background-image: radial-gradient(circle, #ffffff 1px, transparent 1px); background-size: 14px 14px;"></div>
            <div class="absolute -right-10 -top-10 w-40 h-40 border-[24px] border-white/10 rounded-full pointer-events-none"></div>
            <div class="absolute right-20 -bottom-12 w-32 h-32 bg-white/20 rounded-full blur-2xl pointer-events-none"></div>
        ...
        <div class="flex gap-4 overflow-x-auto hide-scrollbar snap-x pb-6 pt-2">
            ${activeVouchers.map((v) => {
                ...
                <div class="w-[280px] sm:w-[320px] shrink-0 snap-start relative group cursor-pointer active:scale-95 transition-transform duration-300" onclick="copyVoucher('${esc(v.code)}')">
                    <div class="w-full h-[110px] bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-primary-dark)] rounded-[1.25rem] shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex relative overflow-hidden border border-white/20">
    ...
    const bLModal = [{name:'Semua Merek', img:appData.store.allBrandsIcon||'https://placehold.co/150/3b82f6/ffffff?text=Semua+Merek'}, ...(appData.brands || [])];
...
// FITUR BARU: Muat skrip AdSense secara dinamis — HANYA kalau iklan diaktifkan admin.
// FIX KRITIS: sebelumnya skrip ini dimuat statis & SELALU aktif di <head>, terlepas dari
// toggle ON/OFF kita. Itu berbahaya karena begitu skrip Google ini dimuat, fitur "Auto Ads"
...
window.rCat = () => {
    ...
    if (backBtnContainer) {
        if (isFiltered) {
            ...
            else if (aBrand !== 'Semua Merek') { filterLabel = "Merek Pilihan"; filterValue = aBrand; filterIcon = "fa-tag"; iconColor = "text-blue-500 bg-blue-50 dark:bg-blue-900/30"; }
    ...
    c.innerHTML = v.map(p => {
        ...
        const soldBadge = totalSoldCard > 0
            ? `<span class="bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400 px-2 py-0.5 rounded-full text-[8px] font-black flex items-center gap-1 whitespace-nowrap uppercase tracking-wider"><i class="fa-solid fa-fire-flame-curved text-orange-400"></i> ${totalSoldCard} Terjual</span>`
            : '';
        ...
        let bH = `<div class="mb-2.5 flex flex-wrap gap-1.5 items-center overflow-hidden shrink-0">
            ...
            ${p.brand ? `<span class="bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-300 px-2 py-0.5 rounded-full text-[8px] font-black flex items-center gap-1 whitespace-nowrap uppercase tracking-wider"><i class="fa-solid fa-tag"></i> ${esc(p.brand)}</span>` : ''}
        ...
        if (cView === 'grid') {
            ...
            <div class="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/80 rounded-[1.25rem] shadow-sm ${cardCursorCls} transition-all duration-300 flex flex-col group relative overflow-hidden" onclick="openProductModal(${p.id})">
                ${nH}
                <div class="relative aspect-square w-full bg-white flex items-center justify-center p-3 sm:p-5 shrink-0 border-b border-slate-100 dark:border-slate-700/50">
                    ${stockBadge}
...
// KONTROL HARGA: Tampilkan teks "Pilih Varian" jika warna belum dipilih
    ...
    let bH = ``;
    if (p.sku) bH += `<span class="bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400 px-2.5 py-1 rounded-full text-[9px] font-black flex items-center gap-1.5 whitespace-nowrap tracking-wider"><i class="fa-solid fa-barcode"></i> ${esc(p.sku)}</span>`;
    ...
    if (p.brand) bH += `<span class="bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300 px-2.5 py-1 rounded-full text-[9px] font-black flex items-center gap-1.5 whitespace-nowrap uppercase tracking-wider"><i class="fa-solid fa-tag"></i> ${esc(p.brand)}</span>`;
    ...
    if (totalSoldDisplay > 0) {
        bH += `<span class="bg-slate-100 text-slate-500 dark:bg-slate-800 dark:text-slate-400 px-2.5 py-1 rounded-full text-[9px] font-black flex items-center gap-1.5 whitespace-nowrap uppercase tracking-wider"><i class="fa-solid fa-fire-flame-curved text-orange-400"></i> ${totalSoldDisplay} Terjual</span>`;
    }
                ...
                <div class="p-5 overflow-y-auto flex-1 space-y-6 custom-scrollbar text-sm">
                    ...
                    <div>
                        ...
                        <a href="${esc(d.buktiPayment)}" target="_blank" class="block rounded-2xl overflow-hidden border-2 border-violet-200 dark:border-violet-800 hover:border-violet-400 transition-colors shadow-sm">
                            <img src="${esc(d.buktiPayment)}" alt="Bukti Pembayaran" class="w-full max-h-56 object-cover" onerror="this.style.display='none'"></i>
...
window.rMemberModalBody = () => {
    ...
    const rewardsHtml = activeRewards.length ? activeRewards.map(r => {
        ...
        <div class="flex items-center gap-3 p-4 rounded-[1.25rem] border ${isSelected ? 'border-violet-400 bg-violet-50 dark:bg-violet-900/20' : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50'}">
            ${r.img ? `<img src="${esc(r.img)}" class="w-14 h-14 rounded-xl object-cover border border-slate-200 dark:border-slate-700 shrink-0" onerror="this.style.display='none'"></i>` : `<div class="w-14 h-14 rounded-xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-300 shrink-0"><i class="fa-solid fa-gift text-xl"></i></div>`}
            ...
            <div class="p-5 sm:p-6 overflow-y-auto flex-1 space-y-5">
                ...
                <div>
                    ...
                    <div id="review-photo-preview-wrap" class="hidden mb-2.5 relative w-24 h-24">
                        <img id="review-photo-preview" class="w-24 h-24 rounded-xl object-cover border border-slate-200 dark:border-slate-700"></i>
    ...
    try {
        ...
        const list = reviews.map(r => {
            ...
            <div class="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-700/60">
                ...
                ${r.photoUrl ? `<img src="${esc(r.photoUrl)}" onclick="window.open('${esc(r.photoUrl)}','_blank')" class="w-20 h-20 rounded-xl object-cover border border-slate-200 dark:border-slate-700 cursor-pointer mb-2" onerror="this.style.display='none'"></i>` : ''}
...
window.checkAdminAccess = () => {
    // SECURITY PATCH: Re-validasi status PRO dari Firestore setiap kali akses admin
    // Mencegah bypass localStorage (localStorage.setItem("isFreshmartPro","true"))
    if (window.isAdm) {
...
window.openAdminTab = (t, fH=false) => {
    ...
    const lockedTabs = ['settings','banks','banners','vouchers'];
    if(!window.isPro && lockedTabs.includes(t)){
        ...
        setH('admin-content', `<div class="text-center py-12 bg-white dark:bg-slate-800 rounded-[2rem] border border-amber-200 dark:border-amber-900/30 shadow-sm px-8 max-w-lg mx-auto mt-6"><div class="w-20 h-20 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 text-amber-500 rounded-full flex items-center justify-center mx-auto mb-5"><i class="fa-solid fa-lock text-4xl"></i></div><h2 class="text-xl font-black text-slate-900 dark:text-white mb-3">Fitur Terkunci (Versi PRO)</h2><p class="text-sm text-slate-500 dark:text-slate-400 font-medium mb-8">Menu <b>${t.toUpperCase()}</b> eksklusif untuk pengguna PRO.</p><div class="max-w-sm mx-auto flex flex-col gap-4"><input autocomplete='off' id="pro-license-input" type="text" placeholder="KODE LISENSI..." class="admin-input !py-4 text-center uppercase tracking-widest font-black text-sm"></i><button onclick="activatePro()" class="w-full bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-500 hover:to-amber-600 text-amber-950 font-black py-4 rounded-xl shadow-lg shadow-amber-500/30 transition-all text-sm"><i class="fa-solid fa-key mr-2"></i> Aktivasi</button></div></div>`);
        return;
    }
...
// Perbaikan Fungsi Pengecekan Lisensi Firebase
window.activatePro = async () => {
    // Sesuaikan ID dengan yang ada di UI modal terkunci
    const inputCode = document.getElementById('pro-license-input').value.trim(); 
    
    if (!inputCode) {
        showToast("Masukkan kode lisensi!");
        return;
    }
    ...
    try {
        ...
        if (docSnap.exists) {
            ...
            if (data.status === "active") {
                ...
                // Simpan kode lisensi ke appData
                appData.licenseKey = inputCode;
                await saveApp(['licenseKey']);
                
                // Muat ulang tab admin yang tadi terkunci
                openAdminTab(cTab);
            } else {
                showToast("Kode lisensi ini sudah tidak aktif.");
            }
        } else {
            showToast("Kode Lisensi tidak valid atau tidak ditemukan!");
        }
    } catch (error) {
        console.error("Error cek lisensi:", error);
        showToast("Gagal menghubungi server database.");
    } finally {
        hLoad(); // Tutup loading animation
    }
}
...
window.saveAdminSettings = async (type) => {
    ...
    try {
        if (type === 'profile') {
            ...
            appData.store.adsEnabled = getV('set-ads-enabled') === 'true';
...
window.rAdmReviews = () => {
    ...
    const list = filtered.map(r => {
        ...
        <div class="p-4 sm:p-5 md:p-6 lg:p-8 rounded-[1.5rem] border shadow-sm ${hidden ? 'border-rose-200 bg-rose-50/40 dark:border-rose-900/40 dark:bg-rose-900/10' : 'border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800'} mb-3">
            ...
            ${r.photoUrl ? `<img src="${esc(r.photoUrl)}" onclick="window.open('${esc(r.photoUrl)}','_blank')" class="w-20 h-20 rounded-xl object-cover border border-slate-200 dark:border-slate-700 cursor-pointer mb-2.5" onerror="this.style.display='none'"></i>` : ''}
...
window.rTaxRenderShell = () => {
    ...
    setH('admin-content', `
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 mb-5">
            <div class="flex gap-1.5 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl overflow-x-auto" id="tax-tab-switcher">
                ${tabs.map(t => `<button onclick="switchTaxTab('${t.k}')" data-tax-tab="${t.k}" class="tax-tab-btn px-3.5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap flex items-center gap-1.5 ${taxActiveTab===t.k?'shadow-sm':'text-slate-500 dark:text-slate-400'}" style="${taxActiveTab===t.k?'background:var(--color-primary);color:#fff':''}"><i class="fa-solid ${t.i}"></i> ${t.l}</button>`).join('')}
            </div>
            <div class="flex gap-2 items-center">
                <select id="tax-year-select" onchange="changeTaxYear(this.value)" class="admin-input !py-2.5 !px-3 text-xs font-bold bg-white dark:bg-slate-800 w-28">
                    ${yearOptions.map(y => `<option value="${y}" ${y===taxYear?'selected':''}>${y}</option>`).join('')}
                </select>
                <select id="tax-month-select" onchange="changeTaxMonth(this.value)" class="admin-input !py-2.5 !px-3 text-xs font-bold bg-white dark:bg-slate-800 w-36">
...
window.switchTaxTab = (tab) => {
    taxActiveTab = tab;
    document.querySelectorAll('.tax-tab-btn').forEach(b => {
        const active = b.dataset.taxTab === tab;
        b.style.background = active ? 'var(--color-primary)' : '';
        b.style.color = active ? '#fff' : '';
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 lg:gap-7">
                ...
                <div>
                    ...
                    <div class="flex gap-2.5 items-center">
                        ${v.img ? `<img src="${esc(v.img)}" class="w-11 h-11 rounded-xl object-cover border-2 border-slate-200 dark:border-slate-600 shrink-0 shadow-sm" onerror="this.style.display='none'"></i>` : ''}
...
// --- FITUR AUTO-LOGIN (Sesi Permanen Firebase) ---
    ...
    if (user) {
        ...
        // 1. REAL-TIME VALIDASI LISENSI DARI FIREBASE (CEK STATUS INSTAN)
        try {
